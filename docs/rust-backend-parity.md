...
**Status:** Reconciled DRAFT, built in bounded chunks. Worktree `audit/rust-parity-architecture-v2` (base `origin/main` @ 940d559). **Not committed / not pushed.**
...
| Command | Result |
|---|---|
| `timeout 180 make setup` | PASS — installed sindarin v0.0.83 to `~/.sn`; `sn --install` pulled `sindarin-pkg-libs` v0.0.18 (linux-arm64), `-sdk`, `-test`. |
| `timeout 300 make build` | PASS — CMake/Ninja Release → `bin/sn` + `bin/tests`. |
| `timeout 900 python3 scripts/run_tests.py all` | ALL GREEN — unit **1609**, cgen **107**, rgen **45**, rgen-errors **8**, mgen **79**, integration **1142**, integration-errors **58**, explore **224**, explore-errors **11**. |
| host `rustc --version` | `rustc 1.93.1`. |
...
| **Packed / serializable structs** | C struct variants + cgen `struct_packed`, `struct_handle_fields`, `struct_serializable` | — | **REJ** — `rust_target.c:97,103` rejects packed/serializable structs and non-val `mem_mode`; `tests/rgen/errors/heap_owning_*` | Implement C-equivalent behavior; any exception requires explicit user approval | High | **PR-D** |
...
| Type conversions (`int`→`str` etc.) | C cgen/mgen conversion coverage | — | **ABSENT** — no rgen conversion test | Add conversion rgen tests | Med | **PR-C** |
...
| **Instance `as ref`** | C emits pointer params | #54 | **REJ (specific)** — `rust_target.c:1150`; PR #54 states C emits incompatible value args for pointer params (evaluation item) | Implement C-equivalent behavior (fix C pointer-param emission, then enable Rust instance `as ref`); any exception requires explicit user approval | High | **PR-D** |
...
|---|---|---|---|---|---|---|
| Standalone std-based Rust (no C runtime linkage) | C runtime `src/runtime/sn_*.c/h` (`sn_string`, `sn_array`, `sn_thread.h`); runtime cgen tests | #9 | **PARITY-DESIGN** — Rust emits `std::string`, `Vec<T>`, plain structs; no C runtime linkage | Implement C-equivalent runtime behavior; any exception requires explicit user approval | High | **PR-D** |
...
| **Serializable / packed structs** | C `struct_packed`, `struct_handle_fields`, `struct_serializable` cgen; PR #5 fixed C encoder finalization idempotency | #5 | **REJ** — `rust_target.c:97,103` rejects packed/serializable structs | Implement C-equivalent behavior; any exception requires explicit user approval | High | **PR-D** |
...
| PR #9–#56 check/review evidence | All cross-platform **Build checks green**; **no review/comments** on any PR | #9–#56 | **EVIDENCE** — docs-only PR checks may be skipped by `paths-ignore` | — | Low | — |
...
|---|---|---|---|---|
| D1 | **PR #54 — instance-method `as ref` rejected on Rust** | PR #54: "Instance-method `as ref` remains rejected because the C backend currently emits incompatible value arguments for pointer parameters." `rust_target.c` `rust_validate_struct_methods` (~L1074). | A method declared `as ref` whose receiver is a struct: C compiles (emits value/pair args), Rust rejects → a valid-on-C program errors on Rust. | **Start by implementing C-equivalent behavior** (emit Rust receiver as a reference). Keep the PR #54 C-incompatibility as a warning; any exception (deliberate Rust-only behavior) requires **explicit user approval**. |
