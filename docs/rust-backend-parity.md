...
| **`main()` non-void / arguments** | C allows non-void main and main arguments (C model `has_main_args` → `int main(int argc, char **argv)`) | — | **PARTIAL** — this slice supports **zero-argument** `void`/`int` main only; `int` main emits Rust process-exit behavior through a typed closure that has an unconditional fallback `return 0` after the rendered body, reached only on source-body fallthrough (`rust_target.c:1251-1263`); **main arguments remain open/unsupported**: parameterized `main` (typechecker enforces a single `str[]` argv parameter) is now explicitly rejected by the Rust validator (`rust_target.c:1264-1270`); positives `main_int_exit` (with `.exit` expected code `7`) and `main_int_fallthrough` (declaration-only `int` main, empty stdout, default exit **0**, no `.exit` sidecar) + negatives `main_non_int_return` (other non-void return kinds rejected at `rust_target.c:1256`) and `main_parameterized` | Implement the argument-parity slice: convert the `str[]` main parameter to Rust `std::env::args`-style argv handling; add rgen tests | Low | **PR-A** |
...
| Suite | Count | Target |
|---|---|---|
| rgen (Rust behavior) | **47 positive** | Rust |
