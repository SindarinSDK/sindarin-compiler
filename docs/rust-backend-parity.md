...
> These results are the **historical audit baseline** recorded at commit `940d5593b14a99454671b3004ada8592fddf9734` on `audit/rust-parity-architecture-v2`. They document the audit snapshot, **not the current state**: the current rgen suite has grown from the historical **47** to **54** positive tests, while the C-side counts (cgen/mgen/integration/exploratory) are unchanged.
...
| `make test-rgen` | PASS — **47/47** Rust generation tests green (historical audit baseline; **current** rgen is **54**). |
| `make build && make test` (exact, unpiped, unredirected) | PASS — unit **1622**, cgen **107**, rgen **47** (historical; current **54**), rgen-errors **10**, mgen **79**, integration **1142**, integration-errors **58**, exploratory **224**, exploratory-errors **11**. |
...
| Suite | Count | Target |
|---|---|---|
| rgen (Rust behavior) | **54 positive** | Rust |
