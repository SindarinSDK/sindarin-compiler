...
- Every disputed claim from the independent audit was re-verified against actual source/tests before being written in:
  ...
  - Rust param gating: method-param block `rust_target.c:1249-1284`, function-param block `:1364-1392`; `as_ref` restricted to `int`/`long` (`:1377-1380`), heap-free named structs only (`rust_heap_free_named_struct_type`, `:305-311`), relevant uses `:1231-1238`, `:1377-1382`; `rust_validate_expr` (`:542`). CONFIRMED.
...
| E2 | Rust validation gate | `rust_target.c:1302` `rust_validate_model_impl` (globals `:1272`, lambdas `:1273`, threads `:1274`, type_decls `:1275`, native pragmas `:1290`), `:542` expr, `:962` stmt, `:159` structs, `:1149` methods | CONFIRMED |
...
| **sync/memory qualifiers** | C memory qualifiers | #49 | **REJ (specific)** — method `rust_target.c:1256-1275`, function `:1371-1386` reject non-default `mem_qual`/non-`none` `sync_mod` | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
...
| **Array method/type restrictions** | C array method codegen | #37,#38,#41 | **REJ (specific)** — definitions `rust_array_method_supported` (`:208`), `rust_array_concat_type_supported` (`:291`), `rust_array_copy_type_supported` (`:313`); validator uses `rust_target.c:864-910` | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
...
| **Heap-owning copy/concat restrictions** | C `copy_of`/`concat` codegen | — | **REJ (specific)** — `rust_target.c:727-744` (copyOf), `:851-877` (concat/join method/type checks) | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
| **Generics** | C generics support (parser `type_args`) | — | **REJ** — `rust_target.c:1308` (type_decls) | Add Rust generics | High | **PR-E** |
| **Native functions / native structs** | C native codegen; repo rule: never emit `extern` forwards for native fns without bodies | #3 | **REJ** — `rust_target.c:1349-1356` (native fn), `:172` (native struct) | Add native fn FFI + native struct support | High | **PR-H** |
...
| **Native FFI / `#pragma source` / `#pragma include`** | C native codegen; repo rule: never emit `extern` forwards for native fns without bodies | #3 | **REJ** — `rust_target.c:1349-1356` (native fn), `:1283-1290` (pragmas) | Add `extern "C"` FFI + pin rgen-error negatives | High | **PR-H** |
