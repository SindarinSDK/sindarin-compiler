...
| Current `make test-rgen` | PASS — **79/79** Rust behavior cases: **60** successful runs plus **19** expected runtime failures. |
| Current `make build && make test` (exact, unpiped, unredirected) | PASS — unit **1624**, cgen **112**, rgen **79**, rgen-errors **10**, mgen **79**, integration **1165**, integration-errors **58**, exploratory **224**, exploratory-errors **11**. |
...
| E10 | rgen coverage | Historical `940d5593` audit: 45 positive (`tests/rgen/*`) + 8 negative (`tests/rgen/errors/*`); current head: 79 rgen behavior cases (60 successful runs + 19 expected runtime failures) + 10 rgen-errors | CONFIRMED |
| E11 | Baseline green | Baseline table above | CONFIRMED |
| E12 | C-only suites | integration 1165, integration-errors 58, explore 224, explore-errors 11, cgen 112 — all C target; **no `--target rust` integration suite exists** | CONFIRMED |
...
| Suite | Count | Target |
|---|---|---|
| rgen (Rust behavior) | **79 behavior cases** — 60 successful runs + 19 expected runtime failures | Rust |
