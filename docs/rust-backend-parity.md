...
| Rust toolchain availability & compile-stage failures | `rust_check_toolchain` (`rust_target.c:112`); gate invoked at `src/target/target.c:201` only for executable output | #9 | **IBT** — `rust-toolchain` remains a **4-case** suite (`tests/rust-toolchain/basic.sn` + `tests/fixtures/fake_rustc.c`). Case 1 captures length-delimited, binary-safe argv records and covers default/profile/RUSTFLAGS build arguments; cases 2–4 pin missing executable, failed `--version`, and failed-build diagnostics (including exact `Error: rustc failed to build generated source`). | — | Low | **PR-A** |
...
| `SN_RUSTFLAGS` / `-g` / `-p` / build dirs / keep-generated / config | `SN_RUSTFLAGS` getenv (`rust_target.c:2120`); profile flags (`:2122-2126`); `src/target/target.c:232-255` (`.sn/build/<target>/<base>_<pid>/`) | #9 | **IBT (toolchain argv)** — case 1 proves the complete ordered argv region between `--edition=2021` and generated `.rs`: default `-C opt-level=3`; `SN_RUSTFLAGS=-C target-cpu=native` after default profile flags; `-g` debug flags; and `-p` profile flags. It also pins the exact `-g -p` conflict diagnostic with no rustc build invocation. Successful builds remove generated files unless `--keep-generated`; failed builds return before that cleanup and currently retain generated artifacts. **UT / open** for externally visible build-directory layout, `--keep-generated`, and failure-cleanup behavior; `SN_RUSTFLAGS` remains env-only (no `sn.rust.cfg`). | Add build-dir/keep-generated/failure-cleanup behavioral coverage. Literal-argv hardening for `SN_RUSTFLAGS` needs a later explicit semantic decision: this slice intentionally preserves its current shell-fragment semantics. | Med | **PR-A** |
...
| rust-toolchain | **4** (case 1 binary-safe invocation records, default/`SN_RUSTFLAGS`/`-g`/`-p` argv and `-g -p` conflict; case 2 missing executable; case 3 nonzero `--version`; case 4 successful `--version` + nonzero build, via `sn_fake_rustc`) | Rust |
