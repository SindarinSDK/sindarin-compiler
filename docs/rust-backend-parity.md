...
- Every disputed claim from the independent audit was re-verified against actual source/tests before being written in:
  ...
  - Rust param gating: method-param block `rust_target.c:1216-1251`, function-param block `:1364-1392`; `as_ref` restricted to `int`/`long` (`:1377-1380`), heap-free named structs only (`rust_heap_free_named_struct_type`, `:305-311`), relevant uses `:1231-1238`, `:1377-1382`; `rust_validate_expr` (`:542`). CONFIRMED.
  - `match`, `sizeof`, `typeof`, `value_of`, `address_of`, `spread`, `sized_array`, pointer/stepped slices all verified as C-supported + Rust-gap via `rust_validate_expr` (`rust_target.c:542`). CONFIRMED.
...
| E2 | Rust validation gate | `rust_target.c:1269` `rust_validate_model_impl` (globals `:1272`, lambdas `:1273`, threads `:1274`, type_decls `:1275`, native pragmas `:1290`), `:542` expr, `:962` stmt, `:159` structs, `:1149` methods | CONFIRMED |
| E3 | Rust lowering | `rust_target.c` — `rust_lower_checked_arithmetic` (`:1415`), `rust_lower_strings` (`:1463`), `rust_lower_array_searches` (`:1585`), `rust_mark_instance_method_clones` (`:1632`), `rust_lower_instance_method_clones` (`:1703`), `rust_lower_interpolation_formats` (`:1725`), `rust_lower_for_continues` (`:1905`), `rust_lower_integer_ref_parameters` (`:2012`) | CONFIRMED |
| E4 | Rust build/toolchain | `rustc_command`/`rustc_quoted` (`rust_target.c:11,17`), `rust_check_toolchain` (`:85`), `rust_build` (`:2079` build: `rustc --edition=2021`), `SN_RUSTFLAGS` (`:2087`) | CONFIRMED |
...
| **Global variables** | C model key `globals`; C integration covers | — | **REJ** — `rust_target.c:1272` "does not support global variables yet" | Add Rust globals (module-level `static mut`/`OnceLock`) + rgen test | Med | **PR-E** |
| **`type` aliases / `type_decls`** | C `type_decls` model key; parser type-arg handling | — | **REJ** — `rust_target.c:1275` "does not support type declarations yet" | Add Rust `type` alias | Med | **PR-E** |
| Primitive types (`int`/`long`/`int32`/`uint`/`uint32`/`byte`/`bool`/`char`/`double`/`float`/`string`) | C codegen for all; cgen primitive tests | #9 | **PARTIAL** — `rust_type_supported` (`rust_target.c:117-135`) accepts these kinds; `int`/`long` as-ref only (see param rows) | — | Low | M0 done |
| **Packed / serializable structs** | C struct variants + cgen fixtures `struct_packed`, `struct_handle_fields`, `expr_struct_literal_handle`; **serializable source support has no dedicated cgen fixture** (handled via encoder/cleanup path) | — | **REJ** — `rust_target.c:172-179` rejects packed/serializable structs and non-val `mem_mode`; `tests/rgen/errors/heap_owning_*` | Implement C-equivalent behavior; any exception requires explicit user approval | High | **PR-D** |
| **Native structs** | C native struct codegen | #3 | **REJ** — `rust_target.c:172` rejects native structs (native-struct FFI) | Implement C-equivalent behavior (native-struct FFI); any exception requires explicit user approval | High | **PR-H** |
...
| **Numeric compound assign** `x += 1` | C `templates/c/partials/expr/compound_assign.hbs`; cgen covers | — | **REJ (specific)** — `rust_target.c:775-793` whitelists string `+=` only; numeric `+=` is **not implemented** on Rust | Implement numeric compound assignments + rgen tests (implementation gap, not under-tested) | Med | **PR-C** |
| **Increment/decrement** | C `compound_assign.hbs` (inc/dec) | #9 | **REJ (specific)** — `rust_target.c:797-809` restricts inc/dec to variables + struct fields; `arr[i]++` / complex lvalues rejected | Add rgen tests for **supported** inc/dec (variables/struct fields) + implement the missing lvalue forms | Med | **PR-C** |
| Type conversions (`int`→`str` etc.) | C emits runtime conversion macros via the method-call path (`gen_model_expr.c:1414-1440`: `__sn__int_toChar`, `__sn__str_toInt`); **no dedicated cgen fixture** | — | **ABSENT / implementation gap** — Rust has no `toChar`/`toInt`/… conversion-method lowering (only printf-style format-specifier handling exists in `rust_target.c:375-525,1730-1825`) | Implement Rust conversion-method lowering + add rgen tests | Med | **PR-C** |
...
| **Sized struct arrays** | C `sized_array` codegen | — | **REJ (specific)** — `rust_target.c:639-647` requires supported element_type; struct element → rejected | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
| **Pointer / stepped slices** | C pointer param codegen (`mem_*` cgen) | — | **REJ (specific)** — pointer array slices + stepped slices rejected (`rust_target.c:656-677`); PR #54 C-incompatibility (evaluation item) | Implement C-equivalent pointer/stepped-slice emission; any exception requires explicit user approval | High | **PR-D** |
...
| **int32 / uint32 / byte `as ref`** | C supports these scalar types | — | **REJ (specific)** — `rust_target.c:1377-1380` limits `as_ref` to `int`/`long`; `tests/rgen/errors/int32_as_ref_parameter.sn` (int32 only tested; uint32/byte untested) | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
| **Heap-owning struct params** | C refcount/cleanup (`mem_struct_ref_refcount`, `mem_struct_val_cleanup` cgen) | — | **REJ (specific)** — `rust_heap_free_named_struct_type` (`:305-311`), relevant uses `:1231-1238`, `:1377-1382`; 5 `tests/rgen/errors/heap_owning_*` | Add heap-owning (refcount) Rust support | High | **PR-D** |
| **Instance `as ref`** | C emits pointer params | #54 (impl. commit `dbce38e`) | **REJ (specific)** — `rust_target.c:1227-1238`; PR #54 (GitHub description + commit `dbce38e`): C emits incompatible value args for pointer params (evaluation item) | First fix the **shared C pointer-argument contract** (with C regression coverage), then enable the **equivalent Rust reference mapping**; any exception requires explicit user approval | High | **PR-D** |
| **Mutating instance `as val`** | C codegen | #54 | **REJ (specific)** — `rust_target.c:1235-1238` (rust_mutating + as_val); `tests/rgen/errors/mutating_instance_as_val_struct_parameter.sn` | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
| **sync/memory qualifiers** | C memory qualifiers | #49 | **REJ (specific)** — method `rust_target.c:1223-1242`, function `:1371-1386` reject non-default `mem_qual`/non-`none` `sync_mod` | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
...
| **Array method/type restrictions** | C array method codegen | #37,#38,#41 | **REJ (specific)** — definitions `rust_array_method_supported` (`:208`), `rust_array_concat_type_supported` (`:291`), `rust_array_copy_type_supported` (`:313`); validator uses `rust_target.c:831-877` | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
...
| **Heap-owning copy/concat restrictions** | C `copy_of`/`concat` codegen | — | **REJ (specific)** — `rust_target.c:694-711` (copyOf), `:851-877` (concat/join method/type checks) | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-D** |
| **Generics** | C generics support (parser `type_args`) | — | **REJ** — `rust_target.c:1275` (type_decls) | Add Rust generics | High | **PR-E** |
| **Native functions / native structs** | C native codegen; repo rule: never emit `extern` forwards for native fns without bodies | #3 | **REJ** — `rust_target.c:1316-1323` (native fn), `:172` (native struct) | Add native fn FFI + native struct support | High | **PR-H** |
| **Native `#pragma source/include`** | C native C source/include handling | — | **REJ** — `rust_target.c:1283-1290` "native C source/include pragmas" | Implement C-equivalent behavior; any exception requires explicit user approval | Med | **PR-H** |
...
| String / array ownership & cleanup | C `src/cgen/ownership.c` (move/clone); `sn_string.c`; cgen `mem_array_string`, `mem_val_struct_copy` | #13 | **IBT/UT** — `rust_lower_strings` marks `rust_needs_clone` (`rust_target.c:1463-1560`); `tests/rgen/string_operations.*` (implemented-but-under-tested) | Add behavioral clone-vs-move test | Med | **PR-B** |
| `copyOf` deep-copy (heap-free structs) | C `copy_of.hbs`; cgen covers | #42–#48 | **IBT** — `tests/rgen/struct_array_copy_of.*` | — | Low | M0 done |
| **Heap-owning struct params / refcount cleanup** | C `mem_struct_refcount`, `mem_struct_val_cleanup` cgen | — | **REJ (specific)** — `rust_heap_free_named_struct_type` (`:305-311`); 5 `tests/rgen/errors/heap_owning_*` | Add refcount / `Drop`-based cleanup | High | **PR-D** |
| **Serializable / packed structs** | C cgen `struct_packed`, `struct_handle_fields`, `expr_struct_literal_handle`; serializable source support (encoder finalization idempotency, PR #5) — **no dedicated cgen fixture for serializable** | #5 | **REJ** — `rust_target.c:172-179` rejects packed/serializable structs | Implement C-equivalent behavior; any exception requires explicit user approval | High | **PR-D** |
| **Native structs (struct FFI)** | C native struct codegen | #3 | **REJ** — `rust_target.c:172` rejects native structs (handled under native-struct FFI) | Add native-struct FFI support | High | **PR-H** |
...
| **Lambdas / callbacks** | C `expr_lambda_basic`, `expr_lambda_capture_ref` cgen; C integration closure tests | — | **REJ** — `rust_target.c:1273` "does not support closures yet" | Add Rust closure lowering (`Fn`/`FnMut`) | High | **PR-F** |
| **Threads / `lock` / `release`** | C `sn_thread.h`; `templates/c/partials/expr/thread_{spawn,detach,sync}.hbs`; cgen `expr_thread_*` | — | **REJ** — `rust_target.c:1274` "does not support threads yet"; `lock`/`release` stmt kinds unhandled (SIL) | Add `std::thread` + `lock` statement | High | **PR-G** |
| **Native FFI / `#pragma source` / `#pragma include`** | C native codegen; repo rule: never emit `extern` forwards for native fns without bodies | #3 | **REJ** — `rust_target.c:1316-1323` (native fn), `:1283-1290` (pragmas) | Add `extern "C"` FFI + pin rgen-error negatives | High | **PR-H** |
