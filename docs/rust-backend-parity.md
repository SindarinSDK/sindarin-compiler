...
> The historical audit baseline was recorded at commit `940d5593b14a99454671b3004ada8592fddf9734` on `audit/rust-parity-architecture-v2`: unit **1609**, cgen **107**, rgen **45**, rgen-errors **8**, mgen **79**, integration **1142**, integration-errors **58**, explore **224**, and explore-errors **11**. These are the `940d5593` audit-run results, not current-head results. Current head is unit **1624**, cgen **113**, rgen **85** behavior cases (**61** successful runs plus **24** expected runtime failures), rgen-errors **10**, mgen **79**, integration **1171**, integration-errors **58**, explore **224**, and explore-errors **11**.
...
Unsigned widths, remaining signed `int`/`long` operations, unchecked behavior, exact cross-target diagnostic text/status, and `SN_RUSTFLAGS` semantics remain open.
...
Unchecked `uint` behavior remains the native C operator path. Remaining signed `int`/`long` operations, exact cross-target diagnostic text/status, and `SN_RUSTFLAGS` semantics remain open.
...
| Current `make test-rgen` | PASS — **85/85** Rust behavior cases: **61** successful runs plus **24** expected runtime failures. |
| Current `make build && make test` (exact, unpiped, unredirected) | PASS — unit **1624**, cgen **113**, rgen **85**, rgen-errors **10**, mgen **79**, integration **1171**, integration-errors **58**, exploratory **224**, exploratory-errors **11**. |
...
| E10 | rgen coverage | Historical `940d5593` audit: 45 positive (`tests/rgen/*`) + 8 negative (`tests/rgen/errors/*`); current head: 85 rgen behavior cases (61 successful runs + 24 expected runtime failures) + 10 rgen-errors | CONFIRMED |
| E11 | Baseline green | Baseline table above | CONFIRMED |
| E12 | C-only suites | integration 1171, integration-errors 58, explore 224, explore-errors 11, cgen 113 — all C target; **no `--target rust` integration suite exists** | CONFIRMED |
...
| Numeric binary ops (checked) | `sn_arith.h` checked `sn_{add,sub,mul,div,mod}_{int32,byte,uint32,uint}`; `tests/cgen/expr_{int32,byte,uint32,uint}_checked` snapshots all helper calls; 26 C integration cases cover each slice’s boundary-success program plus isolated failures | #9 | **IBT (int32 + byte + uint32 + uint slices)** — `rust_lower_checked_arithmetic` → `checked_{add,sub,mul,div,rem}`; 26 matching `tests/rgen/{int32,byte,uint32,uint}_checked_*` cases snapshot, build, and run boundary successes plus `.panic` failures | Strict checked signed-int32, byte, uint32, and uint contracts are covered: unsigned add/sub/mul trap outside their representable ranges and unsigned div/rem trap on zero; int32 div/rem also trap on `MIN / -1`; successful boundaries are exact. The uint C guards are portable pre-operation predicates, while Rust uses u64 checked methods. Remaining gaps are signed `int`/`long` operations, unchecked/debug-overflow behavior, exact cross-target diagnostic/status parity, and `SN_RUSTFLAGS`. | Med | **PR-B** |
...
| Suite | Count | Target |
|---|---|---|
| rgen (Rust behavior) | **85 behavior cases** — 61 successful runs + 24 expected runtime failures | Rust |
