...
> The historical audit baseline was recorded at commit `940d5593b14a99454671b3004ada8592fddf9734` on `audit/rust-parity-architecture-v2`: unit **1609**, cgen **107**, rgen **45**, rgen-errors **8**, mgen **79**, integration **1142**, integration-errors **58**, explore **224**, and explore-errors **11**. These are the `940d5593` audit-run results, not current-head results. Current head is unit **1624**, cgen **114**, rgen **90** behavior cases (**61** successful runs plus **29** expected runtime failures), rgen-errors **10**, mgen **79**, integration **1177**, integration-errors **58**, explore **224**, and explore-errors **11**.
...
| Current `make test-rgen` | PASS — **90/90** Rust behavior cases: **61** successful runs plus **29** expected runtime failures. |
| Current `make build && make test` (exact, unpiped, unredirected) | PASS — unit **1624**, cgen **114**, rgen **90**, rgen-errors **10**, mgen **79**, integration **1177**, integration-errors **58**, exploratory **224**, exploratory-errors **11**. |
...
| E10 | rgen coverage | Historical `940d5593` audit: 45 positive (`tests/rgen/*`) + 8 negative (`tests/rgen/errors/*`); current head: 90 rgen behavior cases (61 successful runs + 29 expected runtime failures) + 10 rgen-errors | CONFIRMED |
| E11 | Baseline green | Baseline table above | CONFIRMED |
| E12 | C-only suites | integration 1177, integration-errors 58, explore 224, explore-errors 11, cgen 114 — all C target; **no `--target rust` integration suite exists** | CONFIRMED |
...
| String / array ownership & cleanup | C `src/cgen/ownership.c` (move/clone); `sn_string.c`; cgen `mem_array_string`, `mem_index_assign_str`, `mem_val_struct_copy`; C integration `test_string_clone_move_ownership` | #13 | **IBT** — `rust_lower_strings` marks `rust_needs_clone` (`rust_target.c:1496-1593`); `tests/rgen/string_clone_move_ownership.*` snapshots and executes independent string ownership for live-variable assignment, ordinary call arguments, struct-field initialization/extraction, and string-array insertion/extraction | — for these covered paths | Med | **PR-B (landed)** |
...
| Suite | Count | Target |
|---|---|---|
| rgen (Rust behavior) | **90 behavior cases** — 61 successful runs + 29 expected runtime failures | Rust |
