...
`post-tag-source-inventory.json` classifies all 725 added source fixtures, including supporting inputs. All 677 remaining post-tag files under the C-facing fixture directories (47 cgen, 29 mgen and 196 integration/error source files, plus companions/support files) are now relocated unchanged under `post-tag-fixtures/`. Its `relocation.json` records original paths, historical paths and SHA-256. The 66 compatible added C cases are preserved as useful explicit historical regressions alongside incompatible assertions; none is discarded or relabeled as tagged acceptance evidence. Rust rgen/error/toolchain files remain active in the separate Rust runner.

The effective default C corpus now has exactly the tag's 3,183 files under `tests/cgen`, `tests/mgen`, `tests/integration` and `tests/exploratory`, with identical paths and blobs, not merely identical runner code. The 221 C failures above refer to the explicitly historical main corpus, not this default tagged corpus. Tagged fixtures and expectations are untouched. The post-relocation repository `make test` passes with the existing local CMake generator (1608 unit, 107 cgen, 79 mgen, 1141 integration, 58 negatives, 224 exploratory, 11 exploratory negatives); `--format --check` passes. See `evidence/effective-corpus-verification.json` and its full make log. No compiler, tag-runner or oracle changes were needed.
