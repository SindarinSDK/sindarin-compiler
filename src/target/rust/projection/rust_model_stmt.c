...
json_object *rust_gen_model_stmt(Arena *arena, Stmt *stmt, SymbolTable *symbol_table,
    ...
    switch (stmt->type)
    {
        case STMT_EXPR:
        {
            ...
            json_object_object_add(obj, "expr",
                rust_gen_model_expr(arena, stmt->as.expression.expression, symbol_table, arithmetic_mode));
            ...
            if (stmt->as.expression.expression->type == EXPR_THREAD_SPAWN)
            {
                ...
                if (expr_obj && json_object_object_get_ex(expr_obj, "thread_id", &tid_obj))
                {
                    ...
                    int tlen = json_object_array_length(rust_g_model_threads);
                    for (int ti = 0; ti < tlen; ti++)
                    {
                        json_object *tdef = json_object_array_get_idx(rust_g_model_threads, ti);
        ...
        case STMT_VAR_DECL:
        {
            ...
            json_object_object_add(obj, "type",
                rust_gen_model_type(arena, stmt->as.var_decl.resolved_type
                                      ? stmt->as.var_decl.resolved_type
                                      : stmt->as.var_decl.type));
            json_object_object_add(obj, "mem_qual",
                json_object_new_string(rust_gen_model_mem_qual_str(stmt->as.var_decl.mem_qualifier)));
            json_object_object_add(obj, "sync_mod",
                json_object_new_string(rust_gen_model_sync_mod_str(stmt->as.var_decl.sync_modifier)));
            ...
            /* Check if this variable is captured by a lambda (needs promoted storage) */
            {
                ...
                for (int ci = 0; ci < rust_g_captured_var_count; ci++) {
                    if (strcmp(rust_g_captured_vars[ci], vname) == 0) { captured = true; break; }
                }
                if (captured)
                          ...
                          : stmt->as.var_decl.type;
            {
                if (vtype)
                {
                    const char *cleanup_kind = rust_gen_model_var_cleanup_kind(vtype, rust_g_suppress_local_cleanup);
            ...
            {
                json_object *init_expr = rust_gen_model_expr(arena, stmt->as.var_decl.initializer, symbol_table, arithmetic_mode);
                 ...
                 * rvalue (e.g. `build().probs`), rust_gen_model_expr lifts the call into a
                ...
                bool init_is_lifted_member = false;
                {
                    ...
                    init_is_lifted_member = rust_ownership_is_lifted_member(init);
                }
                ...
                if (stmt->as.var_decl.initializer->type == EXPR_LAMBDA)
                {
                    ...
                    /* Find this lambda in rust_g_model_lambdas and mark self-captures */
                    int lam_count = json_object_array_length(rust_g_model_lambdas);
                    for (int li = 0; li < lam_count; li++)
                    {
                        json_object *lam_obj = json_object_array_get_idx(rust_g_model_lambdas, li);
                        json_object *lid_obj;
                     ...
                     * instead of generic sn_auto_ptr so captured heap vars are freed. */
                    {
                        ...
                        if (init_obj)
                        {
                                ...
                                json_object_get_boolean(ref_caps_obj))
                            {
                                ...
                                if (rust_g_closure_var_count < MAX_CLOSURE_VAR_MAP)
                                {
                                    int nlen = stmt->as.var_decl.name.length;
                                    char *ncopy = arena_alloc(arena, nlen + 1);
                                    strncpy(ncopy, stmt->as.var_decl.name.start, nlen);
                                    ncopy[nlen] = '\0';
                                    rust_g_closure_var_names[rust_g_closure_var_count] = ncopy;
                                    rust_g_closure_var_lambda_ids[rust_g_closure_var_count] = lid;
                                    rust_g_closure_var_count++;
                                }
                            }
                        }
                    }
        ...
        case STMT_RETURN:
        {
            ...
            if (stmt->as.return_stmt.value)
            {
                ...
                json_object_object_add(obj, "value",
                    rust_gen_model_expr(arena, stmt->as.return_stmt.value, symbol_table, arithmetic_mode));
                ...
                if (is_return_self)
                {
                    ...
                    if (rv->expr_type)
                        json_object_object_add(obj, "return_self_type",
                            rust_gen_model_type(arena, rv->expr_type));
                }
                if (!is_return_self && rv->type == EXPR_VARIABLE && rv->expr_type)
                {
                    ...
                    else if (rt->kind == TYPE_FUNCTION)
                    {
                        ...
                        if (!rust_g_in_lambda_body && rust_g_captured_var_count > 0)
                        {
                            json_object *cap_vars = json_object_new_array();
                            for (int ci = 0; ci < rust_g_captured_var_count; ci++)
                            {
                                json_object_array_add(cap_vars,
                                    json_object_new_string(rust_g_captured_vars[ci]));
                            }
                            ...
                            for (int ci = 0; ci < rust_g_closure_var_count; ci++)
                            {
                                if ((int)strlen(rust_g_closure_var_names[ci]) == vlen &&
                                    strncmp(rust_g_closure_var_names[ci], vname, vlen) == 0)
                                {
                                    json_object_object_add(obj, "has_closure_escape",
                                        json_object_new_boolean(true));
                                    json_object_object_add(obj, "closure_escape_lambda_id",
                                        json_object_new_int(rust_g_closure_var_lambda_ids[ci]));
                                    break;
                                }
                            }
                        }
                    ...
                    else if (rt->kind == TYPE_STRUCT)
                    {
                        ...
                        {
                            if (rust_gen_model_type_has_heap_fields(rt))
                            {
                                is_owned = true;
                    ...
                    if (is_owned && !is_as_val_struct && rv->type == EXPR_VARIABLE)
                    {
                        ...
                        for (int ii = 0; ii < rust_g_owned_iter_var_count; ii++)
                        {
                            if ((int)strlen(rust_g_owned_iter_var_names[ii]) == vlen &&
                                strncmp(rust_g_owned_iter_var_names[ii], vname, vlen) == 0)
                            {
                                is_owned_foreach_binding = true;
                                break;
                            }
                        }
                        for (int pi = 0; !is_owned_foreach_binding && pi < rust_g_all_param_count; pi++)
                        {
                            if ((int)strlen(rust_g_all_param_names[pi]) == vlen &&
                                strncmp(rust_g_all_param_names[pi], vname, vlen) == 0)
                            {
                                is_owned = false;
                    ...
                    if (is_owned)
                    {
                        ...
                        if (rust_g_model_namespace_prefix)
                        {
                            ...
                            if (rust_g_model_ns_static_var_names)
                            {
                                ...
                                {
                                    if (strcmp(rust_g_model_ns_static_var_names[vi], tvar) == 0)
                            ...
                            if (!found && rust_g_model_ns_instance_var_names)
                            {
                                ...
                                {
                                    if (strcmp(rust_g_model_ns_instance_var_names[vi], tvar) == 0)
                                    { found = true; break; }
                                }
                            }
                ...
                else if (rv->type == EXPR_LAMBDA && !rust_g_in_lambda_body &&
                         rust_g_captured_var_count > 0)
                {
                    json_object_object_add(obj, "is_ownership_transfer",
                        json_object_new_boolean(true));
                    json_object_object_add(obj, "transfer_kind",
                ...
                else if (rv->type == EXPR_MEMBER && rv->expr_type)
                {
                    ...
                    bool obj_is_lifted_call =
                        ...
                        rust_gen_model_type_has_heap_fields(obj_expr->expr_type);
                    ...
                    if (obj_expr && obj_expr->expr_type)
                    {
                        ...
                        if (obj_type->kind == TYPE_STRUCT &&
                            rust_gen_model_type_has_heap_fields(obj_type))
                        {
                 ...
                 * (rust_gen_model_expr.c), so the struct literal emits `sn_array_copy(local)`
            ...
            if (!stmt->as.return_stmt.value && rust_g_in_main_void)
            {
            ...
            if (rust_g_lock_depth > 0)
            {
                json_object *cleanups = json_object_new_array();
                for (int li = rust_g_lock_depth - 1; li >= 0; li--)
                    json_object_array_add(cleanups,
                        json_object_new_string(rust_g_lock_var_names[li]));
        ...
        case STMT_BLOCK:
        {
            ...
            for (int i = 0; i < stmt->as.block.count; i++)
            {
                json_object_array_add(stmts,
                    rust_gen_model_stmt(arena, stmt->as.block.statements[i], symbol_table, arithmetic_mode));
            }
        ...
        case STMT_IF:
        {
            ...
            json_object_object_add(obj, "condition",
                rust_gen_model_expr(arena, stmt->as.if_stmt.condition, symbol_table, arithmetic_mode));
            ...
            if (stmt->as.if_stmt.then_branch->type == STMT_BLOCK)
            {
                json_object_object_add(obj, "then_body",
                    rust_gen_model_stmt(arena, stmt->as.if_stmt.then_branch, symbol_table, arithmetic_mode));
            }
            else
            {
            ...
            if (stmt->as.if_stmt.else_branch)
            {
                if (stmt->as.if_stmt.else_branch->type == STMT_BLOCK)
                {
                    json_object_object_add(obj, "else_body",
                        rust_gen_model_stmt(arena, stmt->as.if_stmt.else_branch, symbol_table, arithmetic_mode));
                }
                else
                {
                    json_object *block = json_object_new_object();
        ...
        case STMT_WHILE:
        {
            ...
            json_object_object_add(obj, "condition",
                rust_gen_model_expr(arena, stmt->as.while_stmt.condition, symbol_table, arithmetic_mode));
            json_object_object_add(obj, "body",
                rust_gen_model_stmt(arena, stmt->as.while_stmt.body, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case STMT_FOR:
        {
            ...
            if (stmt->as.for_stmt.initializer)
            {
                json_object_object_add(obj, "init",
                    rust_gen_model_stmt(arena, stmt->as.for_stmt.initializer, symbol_table, arithmetic_mode));
            }
            if (stmt->as.for_stmt.condition)
            {
                json_object_object_add(obj, "condition",
                    rust_gen_model_expr(arena, stmt->as.for_stmt.condition, symbol_table, arithmetic_mode));
            }
            if (stmt->as.for_stmt.increment)
            {
                json_object_object_add(obj, "increment",
                    rust_gen_model_expr(arena, stmt->as.for_stmt.increment, symbol_table, arithmetic_mode));
            }
            json_object_object_add(obj, "body",
                rust_gen_model_stmt(arena, stmt->as.for_stmt.body, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case STMT_FOR_EACH:
        {
            ...
            if (iter_type != NULL)
            {
                ...
                json_object_object_add(obj, "iterable",
                    rust_gen_model_expr(arena, stmt->as.for_each_stmt.iterable, symbol_table, arithmetic_mode));
                json_object_object_add(obj, "body",
                    rust_gen_model_stmt(arena, stmt->as.for_each_stmt.body, symbol_table, arithmetic_mode));
                ...
                json_object_object_add(obj, "iter_type",
                    rust_gen_model_type(arena, iter_type));
                ...
                json_object_object_add(obj, "element_type",
                    rust_gen_model_type(arena, elem_type));

                /* Element cleanup kind for loop variable (next() returns owned values) */
                if (elem_type)
                {
                    const char *elem_ck = rust_gen_model_var_cleanup_kind(elem_type, false);
                ...
                const char *iter_ck = rust_gen_model_var_cleanup_kind(iter_type, false);
            ...
            else
            {
                ...
                if (elem_type)
                {
                    json_object_object_add(obj, "element_type",
                        rust_gen_model_type(arena, elem_type));
                    json_object_object_add(obj, "element_cleanup_kind",
                        json_object_new_string(rust_gen_model_var_cleanup_kind(elem_type, false)));
                }
                ...
                ncopy[nlen] = '\0';
                if (rust_g_owned_iter_var_count % 8 == 0) {
                    char **nv = arena_alloc(arena, (rust_g_owned_iter_var_count + 8) * sizeof(char *));
                    for (int j = 0; j < rust_g_owned_iter_var_count; j++) nv[j] = rust_g_owned_iter_var_names[j];
                    rust_g_owned_iter_var_names = nv;
                }
                rust_g_owned_iter_var_names[rust_g_owned_iter_var_count++] = ncopy;

                json_object_object_add(obj, "body",
                    rust_gen_model_stmt(arena, stmt->as.for_each_stmt.body, symbol_table, arithmetic_mode));

                rust_g_owned_iter_var_count--;
        ...
        case STMT_TYPE_DECL:
        {
            ...
            json_object_object_add(obj, "type",
                rust_gen_model_type(arena, stmt->as.type_decl.type));
            break;
        }
        ...
        case STMT_FUNCTION:
        {
            ...
            json_object_object_add(obj, "function",
                rust_gen_model_function(arena, &stmt->as.function, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case STMT_LOCK:
        {
            ...
            json_object *lock_expr = rust_gen_model_expr(arena, stmt->as.lock_stmt.lock_expr, symbol_table, arithmetic_mode);
            ...
            if (lock_name && rust_g_lock_depth < MAX_LOCK_DEPTH)
                rust_g_lock_var_names[rust_g_lock_depth++] = lock_name;

            json_object_object_add(obj, "body",
                rust_gen_model_stmt(arena, stmt->as.lock_stmt.body, symbol_table, arithmetic_mode));

            /* Pop lock scope */
            if (lock_name && rust_g_lock_depth > 0)
                rust_g_lock_depth--;
            break;
        }
        ...
        case STMT_USING:
        {
            ...
            {
                json_object_object_add(obj, "type",
                    rust_gen_model_type(arena, stmt->as.using_stmt.type));
            }
            json_object_object_add(obj, "initializer",
                rust_gen_model_expr(arena, stmt->as.using_stmt.initializer, symbol_table, arithmetic_mode));
            json_object_object_add(obj, "body",
                rust_gen_model_stmt(arena, stmt->as.using_stmt.body, symbol_table, arithmetic_mode));
            break;
        }
    }
