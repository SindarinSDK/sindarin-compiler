...
static const char *field_copy_action(Type *type)
{
    ...
    switch (type->kind)
    {
        ...
        case TYPE_FUNCTION: return "retain_closure";  /* shared refcounted closure */
        case TYPE_STRUCT:
                              ...
                              ArithmeticMode arithmetic_mode)
{
    ...
    if (decl->name.filename)
        rust_gen_model_add_source_file(obj, decl->name.filename);
    ...
    for (int i = 0; i < decl->field_count; i++)
    {
        ...
        json_object_object_add(field, "type", rust_gen_model_type(arena, f->type));
        ...
        if (f->default_value)
        {
            json_object_object_add(field, "default_value",
                rust_gen_model_expr(arena, f->default_value, symbol_table, arithmetic_mode));
        }
    ...
    for (int i = 0; i < decl->method_count; i++)
    {
        ...
        json_object_object_add(method, "return_type", rust_gen_model_type(arena, m->return_type));
        json_object_object_add(method, "modifier", json_object_new_string(rust_gen_model_func_mod_str(m->modifier)));
        ...
        /* Rust resolved-call validation needs to distinguish overload methods
         * from ordinary methods when projecting borrow qualifiers. This fact
         * belongs only to the Rust model; tagged/shared C output is unchanged. */
        if (m->is_operator)
        ...
        for (int j = 0; j < m->param_count; j++)
        {
            ...
            json_object_object_add(p, "type", rust_gen_model_type(arena, m->params[j].type));
            json_object_object_add(p, "mem_qual",
                json_object_new_string(rust_gen_model_mem_qual_str(m->params[j].mem_qualifier)));
            json_object_object_add(p, "sync_mod",
                json_object_new_string(rust_gen_model_sync_mod_str(m->params[j].sync_modifier)));
            rust_gen_model_emit_param_cleanup(p, &m->params[j], m->is_native);
            json_object_array_add(params, p);
        }
        json_object_object_add(method, "params", params);

        rust_project_body_facts(arena, m->body, m->body_count,
                                m->params, m->param_count, symbol_table);

        /* Method body */
        json_object *body = json_object_new_array();

        /* Save outer as-ref param state and set up method-level as-ref params */
        char **saved_as_ref_names = rust_g_as_ref_param_names;
        int saved_as_ref_count = rust_g_as_ref_param_count;
        ...
        for (int j = 0; j < m->param_count; j++)
        {
            ...
            bool is_as_ref_on_ref_struct = (m->params[j].mem_qualifier == MEM_AS_REF &&
                m->params[j].type && m->params[j].type->kind == TYPE_STRUCT &&
                m->params[j].type->as.struct_type.pass_self_by_ref);
            bool is_default_array_ref =
                m->params[j].mem_qualifier == MEM_DEFAULT && !m->is_native &&
                m->params[j].type && m->params[j].type->kind == TYPE_ARRAY;
            bool is_default_composite_ref =
                m->params[j].mem_qualifier == MEM_DEFAULT && !m->is_native &&
                m->params[j].type && m->params[j].type->kind == TYPE_STRUCT &&
                !m->params[j].type->as.struct_type.pass_self_by_ref &&
                rust_gen_model_type_has_heap_fields(m->params[j].type);
            if (!is_as_ref_on_ref_struct &&
                (m->params[j].mem_qualifier == MEM_AS_REF ||
                 is_default_array_ref || is_default_composite_ref))
            {
                if (rust_g_as_ref_param_count % 8 == 0) {
                    char **nv = arena_alloc(arena, (rust_g_as_ref_param_count + 8) * sizeof(char *));
                    for (int k = 0; k < rust_g_as_ref_param_count; k++) nv[k] = rust_g_as_ref_param_names[k];
                    rust_g_as_ref_param_names = nv;
                }
                rust_g_as_ref_param_names[rust_g_as_ref_param_count++] =
        ...
        if (m->body)
        {
            for (int j = 0; j < m->body_count; j++)
            {
                json_object_array_add(body,
                    rust_gen_model_stmt(arena, m->body[j], symbol_table, arithmetic_mode));
            }
        }
        json_object_object_add(method, "body", body);

        /* Restore outer as-ref param state */
        rust_g_as_ref_param_names = saved_as_ref_names;
        rust_g_as_ref_param_count = saved_as_ref_count;
