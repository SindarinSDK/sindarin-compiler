...
#ifndef RUST_GEN_MODEL_H
#define RUST_GEN_MODEL_H
...
static inline void rust_gen_model_add_source_file(json_object *obj, const char *filename)
{
    if (!filename) return;
    char buf[1024];
...
extern json_object *rust_g_model_lambdas;

/* Global thread collection - populated during model building */
extern json_object *rust_g_model_threads;
extern int rust_g_model_thread_count;
extern int rust_g_model_lambda_count;
...
extern int rust_g_prescan_function_entry_depth;
...
extern char **rust_g_captured_vars;
extern int rust_g_captured_var_count;
...
extern char **rust_g_as_ref_param_names;
extern int rust_g_as_ref_param_count;
...
extern char **rust_g_all_param_names;
extern int rust_g_all_param_count;
...
extern char **rust_g_owned_iter_var_names;
extern int rust_g_owned_iter_var_count;
...
extern char **rust_g_thread_handle_vars;
extern int rust_g_thread_handle_var_count;

/* Suppress auto-cleanup on local array/string vars (set for functions returning structs with heap fields) */
extern bool rust_g_suppress_local_cleanup;
...
extern bool rust_g_has_pragma_source;
...
extern bool rust_g_in_lambda_body;
...
extern bool rust_g_in_main_void;
...
extern bool rust_g_in_thread_spawn_call;
...
extern char *rust_g_closure_var_names[MAX_CLOSURE_VAR_MAP];
extern int rust_g_closure_var_lambda_ids[MAX_CLOSURE_VAR_MAP];
extern int rust_g_closure_var_count;

/* Global fn-wrapper collection for function-as-parameter wrapping */
extern json_object *rust_g_model_fn_wrappers;
extern int rust_g_model_fn_wrapper_count;
...
extern int rust_g_model_member_lift_count;

/* Current namespace prefix for namespaced imports (NULL when not inside a namespace) */
extern const char *rust_g_model_namespace_prefix;

/* Module-level statement access for callee body analysis */
extern Stmt **rust_g_model_module_stmts;
extern int rust_g_model_module_stmt_count;

/* Module-level variable and function names for the current namespace import.
 * When rust_g_model_namespace_prefix is set and a function body references one of
 * these names, the reference gets namespace-prefixed in the model.
 *
 * Static vars (shared across aliases) use rust_g_model_canonical_prefix.
 * Instance vars (per-alias) use rust_g_model_namespace_prefix. */
extern const char **rust_g_model_ns_static_var_names;
extern int rust_g_model_ns_static_var_count;
extern const char **rust_g_model_ns_instance_var_names;
extern int rust_g_model_ns_instance_var_count;
extern const char **rust_g_model_ns_fn_names;
extern const char **rust_g_model_ns_fn_aliases; /* c_alias for each fn (NULL if none) */
extern int rust_g_model_ns_fn_count;
...
extern const char *rust_g_model_canonical_prefix;

/* Post-processing pass: flatten method chains into sequential statements */
void rust_gen_model_flatten_chains(json_object *model);

/* --- Internal functions (used across gen_model_*.c files) --- */

/* Enum-to-string conversions (shared by stmt, func, struct) */
const char *rust_gen_model_mem_qual_str(MemoryQualifier mq);
const char *rust_gen_model_sync_mod_str(SyncModifier sm);
const char *rust_gen_model_func_mod_str(FunctionModifier fm);
...
const char *rust_gen_model_type_kind_str(TypeKind kind);
bool rust_gen_model_type_has_heap_fields(Type *type);
...
/* Struct emission */
json_object *rust_gen_model_struct(Arena *arena, StructDeclStmt *decl, SymbolTable *symbol_table,
                              ArithmeticMode arithmetic_mode);
...
extern const char *rust_g_lock_var_names[MAX_LOCK_DEPTH];
extern int rust_g_lock_depth;

/* Declaration facts belong to the Rust projection, not the typed AST. */
typedef struct {
    SyncModifier sync_modifier;
