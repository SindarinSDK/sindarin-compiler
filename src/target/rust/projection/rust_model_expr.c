    ...
    case EXPR_VARIABLE: {
        ...
        if (!mc_is_param(lam, name) && !mc_is_local(locals, name)) {
            ...
            if (expr->as.variable.declaration_scope_depth > 0 &&
                expr->as.variable.declaration_scope_depth <= rust_g_prescan_function_entry_depth)
                break;
            if (expr->expr_type)
        ...
        /* Also capture the assignment target, unless it is a module-level global */
        {
            ...
            if (!mc_is_param(lam, name) && !mc_is_local(locals, name) && expr->expr_type &&
                !(expr->as.assign.lhs_scope_depth > 0 &&
                  expr->as.assign.lhs_scope_depth <= rust_g_prescan_function_entry_depth))
                mc_add(caps, arena, name, expr->expr_type);
        }
        break;
...
static bool mc_emit_capture_actions(json_object *cap, Type *type, bool is_ref)
{
    ...
    if (is_ref) {
        ...
        case TYPE_STRUCT:
            ...
            } else if (rust_gen_model_type_has_heap_fields(type)) {
...
static const char *mutation_storage_kind(Expr *target)
{
    ...
    for (int i = 0; i < rust_g_captured_var_count; i++)
        if (mutation_variable_name_equals(base, rust_g_captured_vars[i])) return "captured";
    return "local";
}
...
static bool mutation_base_is_captured(Expr *base)
{
    ...
    for (int i = 0; i < rust_g_captured_var_count; i++)
        if (mutation_variable_name_equals(base, rust_g_captured_vars[i])) return true;
    return false;
}
...
static bool mutation_is_checked_scalar_ref_parameter(Expr *target)
{
    ...
    if (!target || target->type != EXPR_VARIABLE || target != base ||
        ...
        !rust_variable_facts(base).is_parameter ||
        rust_variable_facts(base).param_mem_qualifier != MEM_AS_REF ||
        !target->expr_type)
        return false;
...
/* Set up rust_g_as_ref_param_names for a lambda body emission.  Lambdas share the
...
{
    *saved_names = rust_g_as_ref_param_names;
    *saved_count = rust_g_as_ref_param_count;
    ...
    for (int i = 0; i < lam->param_count; i++)
    {
        ...
        bool is_composite_val = (p->mem_qualifier == MEM_DEFAULT && !lam->is_native &&
                                 p->type && p->type->kind == TYPE_STRUCT &&
                                 !p->type->as.struct_type.pass_self_by_ref &&
                                 rust_gen_model_type_has_heap_fields(p->type));
        ...
        if (rust_g_as_ref_param_count % 8 == 0) {
            ...
            for (int j = 0; j < rust_g_as_ref_param_count; j++) nv[j] = rust_g_as_ref_param_names[j];
            rust_g_as_ref_param_names = nv;
        }
        rust_g_as_ref_param_names[rust_g_as_ref_param_count++] = ncopy;
    }
}

static void pop_lambda_as_ref_params(char **saved_names, int saved_count)
{
    rust_g_as_ref_param_names = saved_names;
    rust_g_as_ref_param_count = saved_count;
}
 ...
 *
 * Returns the wrapper_id appended to rust_g_model_fn_wrappers if a wrap was
                                     ...
                                     Type *fn_type, SymbolTable *symbol_table)
{
    ...
    int wrap_id = rust_g_model_fn_wrapper_count++;
    ...
    {
        json_object *pt = rust_gen_model_type(arena, fn_type->as.function.param_types[p]);
        if (!target_is_native)
        {
    ...
    json_object_array_add(rust_g_model_fn_wrappers, wrapper);
    return wrap_id;
}
    ...
    return operand &&
        (operand->type == EXPR_SIZED_ARRAY_ALLOC ||
         (operand->type == EXPR_VARIABLE &&
          rust_variable_facts(operand).declared_as_sized_array));
}
                            ...
                            ArithmeticMode arithmetic_mode)
{
    ...
    if (expr->expr_type)
    {
        json_object_object_add(obj, "type", rust_gen_model_type(arena, expr->expr_type));
    }
    ...
    switch (expr->type)
    {
        ...
        case EXPR_VARIABLE:
        {
            ...
            if (rust_g_model_namespace_prefix)
            {
                bool found = false;
                const char *prefix_to_use = rust_g_model_namespace_prefix;
                /* Check static vars first (use canonical prefix) */
                if (rust_g_model_ns_static_var_names)
                {
                    for (int vi = 0; vi < rust_g_model_ns_static_var_count; vi++)
                    {
                        if (strcmp(rust_g_model_ns_static_var_names[vi], vname) == 0)
                        { found = true; prefix_to_use = rust_g_model_canonical_prefix; break; }
                    }
                }
                /* Check instance vars (use namespace prefix) */
                if (!found && rust_g_model_ns_instance_var_names)
                {
                    for (int vi = 0; vi < rust_g_model_ns_instance_var_count; vi++)
                    {
                        if (strcmp(rust_g_model_ns_instance_var_names[vi], vname) == 0)
                        { found = true; break; }
                    }
                }
                /* Check function names (use namespace prefix, or c_alias if available) */
                if (!found && rust_g_model_ns_fn_names)
                {
                    for (int fi = 0; fi < rust_g_model_ns_fn_count; fi++)
                    {
                        if (strcmp(rust_g_model_ns_fn_names[fi], vname) == 0)
                        {
                            found = true;
                            /* If this function has a c_alias (e.g., native fn with @alias),
                             * use the alias directly instead of namespace-prefixing */
                            if (rust_g_model_ns_fn_aliases && rust_g_model_ns_fn_aliases[fi])
                            {
                                char callable[512];
                                ...
                                json_object_object_add(obj, "name",
                                    json_object_new_string(rust_g_model_ns_fn_aliases[fi]));
             ...
             * or is an 'as ref' parameter (a C pointer that must be dereferenced). */
            {
                ...
                for (int ci = 0; ci < rust_g_captured_var_count; ci++) {
                    if (strcmp(rust_g_captured_vars[ci], stored_name) == 0) {
                        mark_captured = true;
                        break;
                    }
                }
                if (!mark_captured) {
                    for (int ri = 0; ri < rust_g_as_ref_param_count; ri++) {
                        if (strcmp(rust_g_as_ref_param_names[ri], stored_name) == 0) {
                            mark_captured = true;
                            break;
                        }
                    }
        ...
        case EXPR_ASSIGN:
        {
            ...
            if (rust_g_model_namespace_prefix)
            {
                ...
                if (rust_g_model_ns_static_var_names)
                {
                    for (int vi = 0; vi < rust_g_model_ns_static_var_count; vi++)
                    {
                        if (strcmp(rust_g_model_ns_static_var_names[vi], aname) == 0)
                        { found = true; prefix_to_use = rust_g_model_canonical_prefix; break; }
                    }
                }
                if (!found && rust_g_model_ns_instance_var_names)
                {
                    for (int vi = 0; vi < rust_g_model_ns_instance_var_count; vi++)
                    {
                        if (strcmp(rust_g_model_ns_instance_var_names[vi], aname) == 0)
                        { found = true; break; }
                    }
                }
            ...
            json_object_object_add(obj, "value",
                rust_gen_model_expr(arena, expr->as.assign.value, symbol_table, arithmetic_mode));
            /* Check if target is captured or is an 'as ref' param (C pointer needing *) */
            {
                bool mark_captured = false;
                for (int ci = 0; ci < rust_g_captured_var_count; ci++) {
                    if (strcmp(rust_g_captured_vars[ci], aname) == 0) {
                        mark_captured = true;
                        break;
                    }
                }
                if (!mark_captured) {
                    for (int ri = 0; ri < rust_g_as_ref_param_count; ri++) {
                        if (strcmp(rust_g_as_ref_param_names[ri], aname) == 0) {
                            mark_captured = true;
                            break;
                        }
                    }
            ...
            if (expr->expr_type)
            {
                ...
                bool assign_rhs_is_lifted_member = false;
                {
                    ...
                    assign_rhs_is_lifted_member = rust_ownership_is_lifted_member(val);
                }
                ...
                switch (atype->kind)
                {
                    ...
                    case TYPE_STRUCT:
                        ...
                        else
                        {
                            ...
                            if (rust_gen_model_type_has_heap_fields(atype))
                ...
                if (atype->kind == TYPE_STRUCT)
                    json_object_object_add(obj, "target_type",
                        rust_gen_model_type(arena, atype));
            }
            break;
        }
        ...
        case EXPR_COMPOUND_ASSIGN:
        {
            ...
            json_object *ca_target = rust_gen_model_expr(arena, expr->as.compound_assign.target,
            ...
            if (ca_base && rust_variable_facts(ca_base).is_parameter)
            {
                ...
                json_object_object_add(ca_target, "parameter_mem_qual",
                    json_object_new_string(rust_gen_model_mem_qual_str(
                        rust_variable_facts(ca_base).param_mem_qualifier)));
            }
            ...
            json_object *ca_val = rust_gen_model_expr(arena, expr->as.compound_assign.value, symbol_table, arithmetic_mode);
        ...
        case EXPR_BINARY:
        {
            ...
            if (expr->as.binary.operator_method != NULL)
            {
                ...
                if (expr->as.binary.is_derived_operator)
                {
                    ...
                    if (expr->expr_type)
                        json_object_object_add(mc_obj, "type",
                            rust_gen_model_type(arena, expr->expr_type));
                ...
                json_object_object_add(mc_obj, "object",
                    rust_gen_model_expr(arena, mc_object_expr, symbol_table, arithmetic_mode));
                ...
                json_object_array_add(op_args,
                    rust_gen_model_expr(arena, mc_arg_expr, symbol_table, arithmetic_mode));
                ...
                expr->expr_type && expr->expr_type->kind == TYPE_STRING)
            {
                ...
                if (part_count > 2)
                {
                    ...
                    {
                        json_object *part = rust_gen_model_expr(arena, parts.items[pi],
                            symbol_table, arithmetic_mode);
            ...
            json_object *left_obj = rust_gen_model_expr(arena, expr->as.binary.left, symbol_table, arithmetic_mode);
            json_object *right_obj = rust_gen_model_expr(arena, expr->as.binary.right, symbol_table, arithmetic_mode);
        ...
        case EXPR_UNARY:
        {
            ...
            json_object_object_add(obj, "operand",
                rust_gen_model_expr(arena, expr->as.unary.operand, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case EXPR_CALL:
        {
            ...
            if (is_len_builtin)
            {
                ...
                json_object_object_add(obj, "object",
                    rust_gen_model_expr(arena, expr->as.call.arguments[0], symbol_table, arithmetic_mode));
            ...
            else
            {
                    ...
                    if (root && root->type == EXPR_VARIABLE && root->expr_type == NULL)
                    {
                        ...
                        if (rust_g_model_namespace_prefix)
                        {
                            snprintf(final_ns_fn_name, sizeof(final_ns_fn_name), "%s__%s",
                                     rust_g_model_namespace_prefix, ns_fn_name);
                        ...
                        if (expr->as.call.callee->expr_type)
                            json_object_object_add(callee_obj, "type",
                                rust_gen_model_type(arena, expr->as.call.callee->expr_type));
                ...
                {
                    json_object *callee_model = rust_gen_model_expr(arena, expr->as.call.callee, symbol_table, arithmetic_mode);
                ...
                for (int i = 0; i < expr->as.call.arg_count; i++)
                {
                    json_object *arg = rust_gen_model_expr(arena, expr->as.call.arguments[i], symbol_table, arithmetic_mode);
                     ...
                     * Lvalue args pass &arg; non-lvalue args use a statement-expression temp. */
                    {
                            ...
                            !json_object_object_get_ex(arg, "is_ref_arg", &existing_ref))
                        {
                             ...
                             * rust_gen_model_type.c TYPE_FUNCTION pass_by_ptr annotation and
                             * rust_gen_model_func.c rust_g_as_ref_param_names population). */
                            if (expr->as.call.callee->type == EXPR_MEMBER)
                            {
                                    ...
                                    param_is_default && !rust_g_in_thread_spawn_call)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        json_object_object_add(arg, "borrow_type_name",
                                            json_object_new_string(arg_type->as.struct_type.name));
                                        if (rust_gen_model_type_has_heap_fields(arg_type))
                        ...
                        for (int pi = 0; pi < pcount && pi < expr->as.call.arg_count; pi++)
                        {
                                ...
                                strcmp(pt->as.struct_type.name, ret_name) == 0)
                            {
                                ...
                                json_object_object_add(check, "ptr_expr",
                                    rust_gen_model_expr(arena, expr->as.call.arguments[pi],
                                                   symbol_table, arithmetic_mode));
        ...
        case EXPR_METHOD_CALL:
        {
            ...
            {
                json_object_object_add(obj, "object",
                    rust_gen_model_expr(arena, expr->as.method_call.object, symbol_table, arithmetic_mode));
            }
            ...
            if (expr->as.method_call.struct_type)
            {
                json_object_object_add(obj, "struct_type",
                    rust_gen_model_type(arena, expr->as.method_call.struct_type));
            }
            ...
            json_object *args = json_object_new_array();
            {
                ...
                {
                    json_object *arg = rust_gen_model_expr(arena, expr->as.method_call.args[i], symbol_table, arithmetic_mode);
        ...
        case EXPR_ARRAY:
        {
            ...
            {
                json_object *elem = rust_gen_model_expr(arena, expr->as.array.elements[i], symbol_table, arithmetic_mode);
        ...
        case EXPR_ARRAY_ACCESS:
        {
            ...
            json_object_object_add(obj, "array",
                rust_gen_model_expr(arena, expr->as.array_access.array, symbol_table, arithmetic_mode));
            json_object_object_add(obj, "index",
                rust_gen_model_expr(arena, expr->as.array_access.index, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case EXPR_ARRAY_SLICE:
        {
            ...
            json_object_object_add(obj, "array",
                rust_gen_model_expr(arena, expr->as.array_slice.array, symbol_table, arithmetic_mode));
            if (expr->as.array_slice.start)
                json_object_object_add(obj, "start",
                    rust_gen_model_expr(arena, expr->as.array_slice.start, symbol_table, arithmetic_mode));
            if (expr->as.array_slice.end)
                json_object_object_add(obj, "end",
                    rust_gen_model_expr(arena, expr->as.array_slice.end, symbol_table, arithmetic_mode));
            if (expr->as.array_slice.step)
                json_object_object_add(obj, "step",
                    rust_gen_model_expr(arena, expr->as.array_slice.step, symbol_table, arithmetic_mode));
        ...
        case EXPR_INDEX_ASSIGN:
        {
            ...
            json_object_object_add(obj, "array",
                rust_gen_model_expr(arena, expr->as.index_assign.array, symbol_table, arithmetic_mode));
            json_object_object_add(obj, "index",
                rust_gen_model_expr(arena, expr->as.index_assign.index, symbol_table, arithmetic_mode));
            json_object_object_add(obj, "value",
                rust_gen_model_expr(arena, expr->as.index_assign.value, symbol_table, arithmetic_mode));
        ...
        case EXPR_MEMBER:
        {
                ...
                (obj_type->kind == TYPE_STRING || obj_type->kind == TYPE_ARRAY))
            {
                json_object_object_add(obj, "kind", json_object_new_string("builtin_length"));
                json_object_object_add(obj, "object",
                    rust_gen_model_expr(arena, expr->as.member.object, symbol_table, arithmetic_mode));
            ...
            else
            {
                ...
                if (rust_ownership_is_lifted_member(expr))
                {
                    ...
                    if (ft)
                    {
                                 ...
                                 ft->as.struct_type.pass_self_by_ref)
                        {
                            ...
                            keeper_type_name = ft->as.struct_type.name;
                                 ...
                                 !ft->as.struct_type.pass_self_by_ref &&
                                 rust_gen_model_type_has_heap_fields(ft))
                        {
                            keeper = "val_copy";
                            keeper_type_name = ft->as.struct_type.name;
                        }
                    }
                    int lift_id = rust_g_model_member_lift_count++;
                    char tmp_var[64];
        ...
        case EXPR_MEMBER_ACCESS:
        {
            ...
            json_object_object_add(obj, "object",
                rust_gen_model_expr(arena, expr->as.member_access.object, symbol_table, arithmetic_mode));
        ...
        case EXPR_MEMBER_ASSIGN:
        {
            ...
            json_object_object_add(obj, "object",
                rust_gen_model_expr(arena, expr->as.member_assign.object, symbol_table, arithmetic_mode));
            json_object_object_add(obj, "field_name",
                json_object_new_string(expr->as.member_assign.field_name.start));
            json_object_object_add(obj, "value",
                rust_gen_model_expr(arena, expr->as.member_assign.value, symbol_table, arithmetic_mode));
        ...
        case EXPR_STRUCT_LITERAL:
        {
            ...
            for (int i = 0; i < expr->as.struct_literal.field_count; i++)
            {
                ...
                fv_is_lifted_member = rust_ownership_is_lifted_member(fv);
                            ...
                            sf->type->kind == TYPE_ARRAY && sf->type->as.array.element_type)
                        {
                            ...
                            if (fval_obj)
                            {
                                json_object_object_add(fval_obj, "type",
                                    rust_gen_model_type(arena, sf->type));
            ...
            for (int i = 0; i < expr->as.interpol.part_count; i++)
            {
                ...
                else
                {
                    ...
                    json_object_object_add(part, "expr",
                        rust_gen_model_expr(arena, p, symbol_table, arithmetic_mode));
        ...
        case EXPR_INCREMENT:
        {
            ...
            if (mutation_is_checked_scalar_ref_parameter(expr->as.operand))
                json_object_object_add(obj, "mutation_rhs_first", json_object_new_boolean(true));
            json_object *inc_operand = rust_gen_model_expr(arena, expr->as.operand, symbol_table, arithmetic_mode);
            json_object_object_add(obj, "operand", inc_operand);
            Expr *inc_base = mutation_base_variable(expr->as.operand);
            if (inc_base && rust_variable_facts(inc_base).is_parameter)
            {
                ...
                json_object_object_add(inc_operand, "parameter_mem_qual",
                    json_object_new_string(rust_gen_model_mem_qual_str(
                        rust_variable_facts(inc_base).param_mem_qualifier)));
            }
            json_object *captured_obj = NULL;
        ...
        case EXPR_DECREMENT:
        {
            ...
            if (mutation_is_checked_scalar_ref_parameter(expr->as.operand))
                json_object_object_add(obj, "mutation_rhs_first", json_object_new_boolean(true));
            json_object *dec_operand = rust_gen_model_expr(arena, expr->as.operand, symbol_table, arithmetic_mode);
            json_object_object_add(obj, "operand", dec_operand);
            Expr *dec_base = mutation_base_variable(expr->as.operand);
            if (dec_base && rust_variable_facts(dec_base).is_parameter)
            {
                ...
                json_object_object_add(dec_operand, "parameter_mem_qual",
                    json_object_new_string(rust_gen_model_mem_qual_str(
                        rust_variable_facts(dec_base).param_mem_qualifier)));
            }
            json_object *captured_obj = NULL;
        ...
        case EXPR_RANGE:
        {
            ...
            json_object_object_add(obj, "start",
                rust_gen_model_expr(arena, expr->as.range.start, symbol_table, arithmetic_mode));
            json_object_object_add(obj, "end",
                rust_gen_model_expr(arena, expr->as.range.end, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case EXPR_SPREAD:
        {
            ...
            json_object_object_add(obj, "operand",
                rust_gen_model_expr(arena, expr->as.spread.array, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case EXPR_SIZED_ARRAY_ALLOC:
        {
            ...
            json_object_object_add(obj, "element_type",
                rust_gen_model_type(arena, expr->as.sized_array_alloc.element_type));
            json_object_object_add(obj, "size",
                rust_gen_model_expr(arena, expr->as.sized_array_alloc.size_expr, symbol_table, arithmetic_mode));
            if (expr->as.sized_array_alloc.default_value)
            {
                json_object_object_add(obj, "default_value",
                    rust_gen_model_expr(arena, expr->as.sized_array_alloc.default_value, symbol_table, arithmetic_mode));
            }
            break;
        }
        ...
        case EXPR_LAMBDA:
        {
            ...
            lam->lambda_id = rust_g_model_lambda_count++;
            json_object_object_add(obj, "lambda_id", json_object_new_int(lam->lambda_id));
            json_object_object_add(obj, "return_type", rust_gen_model_type(arena, lam->return_type));
            ...
            for (int i = 0; i < lam->param_count; i++)
            {
                ...
                json_object_object_add(p, "type",
                    rust_gen_model_type(arena, lam->params[i].type));
                json_object_object_add(p, "mem_qual",
                    json_object_new_string(rust_gen_model_mem_qual_str(lam->params[i].mem_qualifier)));
                rust_gen_model_emit_param_cleanup(p, &lam->params[i], lam->is_native);
            ...
            for (int i = 0; i < caps.count; i++)
            {
                ...
                json_object_object_add(cap, "type",
                    rust_gen_model_type(arena, caps.types[i]));
                json_object_object_add(cap, "is_primitive",
                    json_object_new_boolean(mc_is_primitive(caps.types[i])));
                /* is_ref: true if outer var is promoted (in rust_g_captured_vars) */
                bool is_ref = false;
                for (int ci = 0; ci < rust_g_captured_var_count; ci++) {
                    if (strcmp(rust_g_captured_vars[ci], caps.names[i]) == 0) { is_ref = true; break; }
                }
                ...
                if (!is_ref && caps.types[i] &&
                    ...
                    rust_gen_model_type_has_heap_fields(caps.types[i]))
                {
            ...
            /* Check if any capture is by reference (needs heap promotion) */
            {
                ...
                {
                    for (int gi = 0; gi < rust_g_captured_var_count; gi++)
                    {
                        if (strcmp(rust_g_captured_vars[gi], caps.names[ci]) == 0)
            ...
            /* Body */
            {
                bool prev_in_lambda = rust_g_in_lambda_body;
                rust_g_in_lambda_body = true;
                ...
                if (lam->has_stmt_body)
                {
                    ...
                    for (int i = 0; i < lam->body_stmt_count; i++)
                    {
                        json_object_array_add(body,
                            rust_gen_model_stmt(arena, lam->body_stmts[i], symbol_table, arithmetic_mode));
                    }
                ...
                else if (lam->body)
                {
                    json_object_object_add(obj, "body",
                        rust_gen_model_expr(arena, lam->body, symbol_table, arithmetic_mode));
            ...
            if (rust_g_model_lambdas != NULL)
            {
                ...
                if (expr->token && expr->token->filename)
                    rust_gen_model_add_source_file(ldef, expr->token->filename);
                json_object_object_add(ldef, "return_type", rust_gen_model_type(arena, lam->return_type));
                ...
                for (int i = 0; i < lam->param_count; i++)
                {
                    ...
                    json_object_object_add(lp, "mem_qual",
                        json_object_new_string(rust_gen_model_mem_qual_str(lam->params[i].mem_qualifier)));
                    rust_gen_model_emit_param_cleanup(lp, &lam->params[i], lam->is_native);
                ...
                for (int i = 0; i < caps.count; i++)
                {
                    ...
                    for (int ci = 0; ci < rust_g_captured_var_count; ci++) {
                        if (strcmp(rust_g_captured_vars[ci], caps.names[i]) == 0) { lc_is_ref = true; break; }
                    }
                ...
                json_object_array_add(rust_g_model_lambdas, ldef);
            }
            break;
        }
        ...
        case EXPR_MATCH:
        {
            ...
            json_object_object_add(obj, "subject",
                rust_gen_model_expr(arena, match_subj, symbol_table, arithmetic_mode));
            ...
            for (int i = 0; i < expr->as.match_expr.arm_count; i++)
            {
                ...
                for (int j = 0; j < arm->pattern_count; j++)
                {
                    json_object_array_add(patterns,
                        rust_gen_model_expr(arena, arm->patterns[j], symbol_table, arithmetic_mode));
                }
                json_object_object_add(a, "patterns", patterns);
                if (arm->body)
                {
                    json_object_object_add(a, "body",
                        rust_gen_model_stmt(arena, arm->body, symbol_table, arithmetic_mode));
        ...
        case EXPR_THREAD_SPAWN:
        {
            ...
            int thread_id = rust_g_model_thread_count++;
            json_object_object_add(obj, "thread_id", json_object_new_int(thread_id));

            Expr *call_expr = expr->as.thread_spawn.call;
            bool was_in_ts = rust_g_in_thread_spawn_call;
            rust_g_in_thread_spawn_call = true;
            json_object *call_obj = rust_gen_model_expr(arena, call_expr, symbol_table, arithmetic_mode);
            rust_g_in_thread_spawn_call = was_in_ts;
            ...
            if (rust_g_model_threads != NULL)
            {
                ...
                json_object_object_add(tdef, "return_type",
                    rust_gen_model_type(arena, ret_type));
                ...
                if (call_expr->type == EXPR_CALL)
                {
                    ...
                    if (is_method_spawn)
                    {
                        ...
                        json_object_object_add(self_arg, "type",
                            rust_gen_model_type(arena, callee->as.member.object->expr_type));
                    ...
                    for (int i = 0; i < call_expr->as.call.arg_count; i++)
                    {
                        ...
                        json_object_object_add(arg, "type",
                            rust_gen_model_type(arena, arg_expr->expr_type));
                         ...
                         * rust_gen_model_emit_param_cleanup). */
                ...
                json_object_array_add(rust_g_model_threads, tdef);
            }
        ...
        case EXPR_THREAD_SYNC:
        {
            ...
            json_object_object_add(obj, "handle",
                rust_gen_model_expr(arena, expr->as.thread_sync.handle, symbol_table, arithmetic_mode));
                ...
                expr->expr_type && expr->expr_type->kind == TYPE_ARRAY)
            {
                ...
                for (int ti = 0; ti < rust_g_thread_handle_var_count; ti++)
                {
                    if (strcmp(rust_g_thread_handle_vars[ti], vname) == 0)
                    {
                ...
                if (!has_th_companion)
                {
                    ...
                    json_object_object_add(obj, "element_type",
                        rust_gen_model_type(arena, expr->expr_type->as.array.element_type));
                }
            }
        ...
        case EXPR_THREAD_DETACH:
        {
            ...
            json_object_object_add(obj, "handle",
                rust_gen_model_expr(arena, expr->as.thread_detach.handle, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case EXPR_SYNC_LIST:
        {
            ...
            for (int i = 0; i < expr->as.sync_list.element_count; i++)
            {
                ...
                json_object *elem_obj = rust_gen_model_expr(arena, elem, symbol_table, arithmetic_mode);
        ...
        case EXPR_ADDRESS_OF:
        {
            ...
            json_object_object_add(obj, "operand",
                rust_gen_model_expr(arena, expr->as.address_of.operand, symbol_table, arithmetic_mode));
                     ...
                     expr->as.address_of.operand->expr_type->kind == TYPE_ARRAY)
            {
                ...
                if (elem)
                    json_object_object_add(obj, "element_type", rust_gen_model_type(arena, elem));
            }
            break;
        }
        ...
        case EXPR_VALUE_OF:
        {
            ...
            json_object_object_add(obj, "operand",
                rust_gen_model_expr(arena, expr->as.value_of.operand, symbol_table, arithmetic_mode));
        ...
        case EXPR_COPY_OF:
        {
            ...
            json_object_object_add(obj, "operand",
                rust_gen_model_expr(arena, expr->as.copy_of.operand, symbol_table, arithmetic_mode));
            break;
        }
        ...
        case EXPR_SIZEOF:
        {
            ...
            {
                json_object_object_add(obj, "target_type",
                    rust_gen_model_type(arena, expr->as.sizeof_expr.type_operand));
            }
            if (expr->as.sizeof_expr.expr_operand)
            {
                ...
                {
                    json_object_object_add(obj, "target_type",
                        rust_gen_model_type(arena, expr->as.sizeof_expr.expr_operand->expr_type));
                }
            }
            break;
        }
