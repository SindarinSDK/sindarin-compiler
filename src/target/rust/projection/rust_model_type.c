...
void rust_gen_model_emit_param_cleanup(json_object *param_obj, Parameter *param, bool callee_is_native)
{
     ...
     * rust_gen_model_expr.c skips the '&' for ref-struct args regardless of
    ...
    if (param->mem_qualifier == MEM_AS_VAL && !param->type->as.struct_type.pass_self_by_ref)
    {
        if (rust_gen_model_type_has_heap_fields(param->type))
        {
...
json_object *rust_gen_model_type(Arena *arena, Type *type)
{
    ...
    json_object_object_add(obj, "kind", json_object_new_string(rust_gen_model_type_kind_str(type->kind)));
    ...
    switch (type->kind)
    {
        case TYPE_ARRAY:
            ...
            if (type->as.array.element_type)
            {
                json_object_object_add(obj, "element_type",
                    rust_gen_model_type(arena, type->as.array.element_type));
            }
            break;
        ...
        case TYPE_POINTER:
            if (type->as.pointer.base_type)
            {
                json_object_object_add(obj, "base_type",
                    rust_gen_model_type(arena, type->as.pointer.base_type));
            }
            break;
        ...
        case TYPE_FUNCTION:
        {
            json_object_object_add(obj, "return_type",
                rust_gen_model_type(arena, type->as.function.return_type));

            json_object *params = json_object_new_array();
            for (int i = 0; i < type->as.function.param_count; i++)
            {
                Type *pt = type->as.function.param_types[i];
                json_object *pt_obj = rust_gen_model_type(arena, pt);
                /* Flag composite val-struct params on non-native functions as
                 * passed by pointer — mirrors the ABI rewrite in
                 * rust_gen_model_emit_param_cleanup and rust_gen_model_func.c's
                 * rust_g_as_ref_param_names population. Closure-call function
                ...
                if (!type->as.function.is_native && pt &&
                    ...
                    rust_gen_model_type_has_heap_fields(pt))
                {
            ...
            if (type->as.function.param_mem_quals)
            {
                ...
                for (int i = 0; i < type->as.function.param_count; i++)
                {
                     ...
                     * emitter in rust_gen_model_emit_param_cleanup. */
        ...
        case TYPE_GENERIC_INST:
        {
            ...
            if (type->as.generic_inst.resolved != NULL)
            {
                ...
                json_object_put(obj);
                return rust_gen_model_type(arena, type->as.generic_inst.resolved);
            }
