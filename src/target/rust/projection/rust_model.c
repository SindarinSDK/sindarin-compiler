...
json_object *rust_g_model_lambdas = NULL;

/* Global thread collection */
json_object *rust_g_model_threads = NULL;
int rust_g_model_thread_count = 0;
int rust_g_model_lambda_count = 0;

/* Global captured-variable set */
char **rust_g_captured_vars = NULL;
int rust_g_captured_var_count = 0;

/* Global as-ref parameter names (used to emit -> for member access) */
char **rust_g_as_ref_param_names = NULL;
int rust_g_as_ref_param_count = 0;

/* All parameter names of the current function (used to detect borrowed returns) */
char **rust_g_all_param_names = NULL;
int rust_g_all_param_count = 0;

/* Normal-array foreach binding names (owned loop locals) */
char **rust_g_owned_iter_var_names = NULL;
int rust_g_owned_iter_var_count = 0;

/* Global thread-handle-variable set */
char **rust_g_thread_handle_vars = NULL;
int rust_g_thread_handle_var_count = 0;

/* Suppress auto-cleanup on local array/string vars (set for functions returning structs with heap fields) */
bool rust_g_suppress_local_cleanup = false;
bool rust_g_has_pragma_source = false;
bool rust_g_in_lambda_body = false;
bool rust_g_in_main_void = false;
bool rust_g_in_thread_spawn_call = false;

char *rust_g_closure_var_names[MAX_CLOSURE_VAR_MAP];
int rust_g_closure_var_lambda_ids[MAX_CLOSURE_VAR_MAP];
int rust_g_closure_var_count = 0;

/* Lock scope stack */
const char *rust_g_lock_var_names[MAX_LOCK_DEPTH];
int rust_g_lock_depth = 0;

/* Global fn-wrapper collection for function-as-parameter wrapping */
json_object *rust_g_model_fn_wrappers = NULL;
int rust_g_model_fn_wrapper_count = 0;

/* Counter for synthetic struct-rvalue member-access lift temporaries (issue #49) */
int rust_g_model_member_lift_count = 0;

/* Current namespace prefix for namespaced imports */
const char *rust_g_model_namespace_prefix = NULL;

/* Module statements for callee body analysis */
Stmt **rust_g_model_module_stmts = NULL;
int rust_g_model_module_stmt_count = 0;

/* Module-level names for namespace prefixing inside function bodies */
const char **rust_g_model_ns_static_var_names = NULL;
int rust_g_model_ns_static_var_count = 0;
const char **rust_g_model_ns_instance_var_names = NULL;
int rust_g_model_ns_instance_var_count = 0;
const char **rust_g_model_ns_fn_names = NULL;
const char **rust_g_model_ns_fn_aliases = NULL;
int rust_g_model_ns_fn_count = 0;

/* Canonical module prefix for static variable sharing */
const char *rust_g_model_canonical_prefix = NULL;
...
{
    const char *old_prefix = rust_g_model_namespace_prefix;
    const char *old_canonical = rust_g_model_canonical_prefix;
    rust_g_model_namespace_prefix = ns_prefix;
    rust_g_model_canonical_prefix = canonical;
        ...
        if (s->type == STMT_VAR_DECL)
        {
            ...
            if (s->as.var_decl.is_static)
            {
                ...
                if (!already_emitted(prefixed, emitted_names, *emitted_count))
                {
                    ...
                    json_object *gvar = rust_gen_model_stmt(arena, s, symbol_table, arithmetic_mode);
    ...
    if (pragmas)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            json_object_array_add(pragmas,
                rust_gen_model_stmt(arena, s, symbol_table, arithmetic_mode));
        }
    }
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (!already_emitted(sname_buf, emitted_names, *emitted_count))
        {
            ...
            json_object_array_add(structs,
                rust_gen_model_struct(arena, &s->as.struct_decl,
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        rust_g_model_ns_static_var_names = static_var_names;
        rust_g_model_ns_static_var_count = static_var_count;
        rust_g_model_ns_instance_var_names = instance_var_names;
        rust_g_model_ns_instance_var_count = instance_var_count;
        rust_g_model_ns_fn_names = fn_names;
        rust_g_model_ns_fn_aliases = fn_aliases;
        rust_g_model_ns_fn_count = fn_count;
        json_object_array_add(functions,
            rust_gen_model_function(arena, &s->as.function,
                               symbol_table, arithmetic_mode));
        rust_g_model_ns_static_var_names = NULL;
        rust_g_model_ns_static_var_count = 0;
        rust_g_model_ns_instance_var_names = NULL;
        rust_g_model_ns_instance_var_count = 0;
        rust_g_model_ns_fn_names = NULL;
        rust_g_model_ns_fn_aliases = NULL;
        rust_g_model_ns_fn_count = 0;
    }
    ...
    rust_g_model_namespace_prefix = old_prefix;
    rust_g_model_canonical_prefix = old_canonical;
}

json_object *rust_gen_model_build(Arena *arena, Module *module, SymbolTable *symbol_table,
                             ArithmeticMode arithmetic_mode)
{
    rust_variable_facts_reset();
    rust_g_model_module_stmts = module->statements;
    rust_g_model_module_stmt_count = module->count;
    ...
    rust_g_model_lambdas = json_object_new_array();

    /* Initialize global thread collection */
    rust_g_model_threads = json_object_new_array();
    rust_g_model_thread_count = 0;
    rust_g_model_lambda_count = 0;

    /* Initialize global fn-wrapper collection */
    rust_g_model_fn_wrappers = json_object_new_array();
    rust_g_model_fn_wrapper_count = 0;

    /* Reset member-lift counter */
    rust_g_model_member_lift_count = 0;
     ...
     * symbols are gone.  The closure-call detector in rust_gen_model_expr looks them up:
    ...
    for (int i = 0; i < module->count; i++)
    {
        ...
        switch (stmt->type)
        {
            ...
            {
                json_object_array_add(pragmas,
                    rust_gen_model_stmt(arena, stmt, symbol_table, arithmetic_mode));
                break;
            }
            ...
            case STMT_VAR_DECL:
            {
                json_object *gvar = rust_gen_model_stmt(arena, stmt, symbol_table, arithmetic_mode);
                /* Source file tracking for modular compilation */
                if (stmt->as.var_decl.name.filename)
                    rust_gen_model_add_source_file(gvar, stmt->as.var_decl.name.filename);
     ...
     * Re-running the body type-check immediately before codegen refreshes expr_type. */
    {
        ...
        for (int i = 0; i < inst_count; i++)
        {
            ...
            json_object_array_add(structs,
                rust_gen_model_struct(arena, inst->instantiated_decl,
                                 symbol_table, arithmetic_mode));
        }
    }
     ...
     * expr_type with the correct concrete types for this instantiation. */
    {
        ...
        for (int i = 0; i < fn_inst_count; i++)
        {
            ...
            json_object_array_add(functions,
                rust_gen_model_function(arena, inst->instantiated_decl,
                                   symbol_table, arithmetic_mode));
        }
    }
    ...
    json_object_object_add(root, "lambdas", rust_g_model_lambdas);
    rust_g_model_lambdas = NULL;
    json_object_object_add(root, "threads", rust_g_model_threads);
    rust_g_model_threads = NULL;
    json_object_object_add(root, "fn_wrappers", rust_g_model_fn_wrappers);
    rust_g_model_fn_wrappers = NULL;

    return root;
}

int rust_gen_model_write(json_object *model, const char *output_path)
{
