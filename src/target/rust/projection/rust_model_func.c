...
int rust_g_prescan_function_entry_depth = 0;

typedef struct RustFactEntry {
    const Expr *expr;
    RustVariableFacts facts;
    struct RustFactEntry *next;
} RustFactEntry;
static RustFactEntry *variable_facts;

void rust_variable_facts_reset(void) { variable_facts = NULL; }

RustVariableFacts rust_variable_facts(const Expr *expr)
{
    for (RustFactEntry *entry = variable_facts; entry; entry = entry->next)
        if (entry->expr == expr) return entry->facts;
    return (RustVariableFacts){0};
}
...
static void record_variable_facts(Arena *arena, Expr *expr, SymbolTable *table)
{
    ...
    if (sym) {
        ...
        entry->facts.declared_as_sized_array = sym->var_decl_origin &&
    ...
    entry->next = variable_facts;
    variable_facts = entry;
}

static void register_parameter(SymbolTable *table, Parameter *param)
{
    symbol_table_add_symbol_full(table, param->name, param->type,
                                SYMBOL_PARAM, param->mem_qualifier);
    Symbol *sym = symbol_table_lookup_symbol(table, param->name);
    if (sym) sym->sync_mod = param->sync_modifier;
}
...
static void prescan_add_captured(Arena *arena, const char *name)
{
    for (int i = 0; i < rust_g_captured_var_count; i++)
        if (strcmp(rust_g_captured_vars[i], name) == 0) return;
    /* Simple grow - allocates from arena */
    if (rust_g_captured_var_count % 8 == 0) {
        char **nv = arena_alloc(arena, (rust_g_captured_var_count + 8) * sizeof(char *));
        for (int i = 0; i < rust_g_captured_var_count; i++) nv[i] = rust_g_captured_vars[i];
        rust_g_captured_vars = nv;
    }
    rust_g_captured_vars[rust_g_captured_var_count++] = arena_strdup(arena, name);
}
...
static bool prescan_is_outer_var(SymbolTable *table, Token name, int lambda_scope_depth)
{
    ...
    if (sym->declaration_scope_depth <= rust_g_prescan_function_entry_depth) return false;
...
static void prescan_expr(Arena *arena, Expr *expr, SymbolTable *table, int lambda_scope_depth)
{
    ...
    case EXPR_LAMBDA: {
        ...
        for (int i = 0; i < lam->param_count; i++)
            register_parameter(table, &lam->params[i]);
    ...
    case EXPR_ADDRESS_OF:
        prescan_expr(arena, expr->as.address_of.operand, table, lambda_scope_depth);
        break;
    case EXPR_VALUE_OF:
        prescan_expr(arena, expr->as.value_of.operand, table, lambda_scope_depth);
        break;
    case EXPR_COPY_OF:
        prescan_expr(arena, expr->as.copy_of.operand, table, lambda_scope_depth);
        break;
    case EXPR_MEMBER_ASSIGN:
        prescan_expr(arena, expr->as.member_assign.object, table, lambda_scope_depth);
        prescan_expr(arena, expr->as.member_assign.value, table, lambda_scope_depth);
        break;
    case EXPR_STRUCT_LITERAL:
        for (int i = 0; i < expr->as.struct_literal.field_count; i++)
            prescan_expr(arena, expr->as.struct_literal.fields[i].value, table, lambda_scope_depth);
        break;
    case EXPR_SIZED_ARRAY_ALLOC:
        prescan_expr(arena, expr->as.sized_array_alloc.size_expr, table, lambda_scope_depth);
        prescan_expr(arena, expr->as.sized_array_alloc.default_value, table, lambda_scope_depth);
        break;
...
static void prescan_stmt(Arena *arena, Stmt *stmt, SymbolTable *table, int lambda_scope_depth)
{
    ...
    case STMT_VAR_DECL:
                                     ...
                                     SYMBOL_LOCAL, stmt->as.var_decl.mem_qualifier);
        {
            ...
            if (sym) {
                sym->sync_mod = stmt->as.var_decl.sync_modifier;
...
static void prescan_function_body(Arena *arena, Stmt **stmts, int stmt_count,
                                  Parameter *params, int param_count, SymbolTable *table)
{
    rust_g_captured_vars = NULL;
    rust_g_captured_var_count = 0;
    rust_g_closure_var_count = 0;
    ...
    rust_g_prescan_function_entry_depth = symbol_table_get_scope_depth(table);
    ...
    symbol_table_push_scope(table);
    for (int i = 0; i < param_count; i++) register_parameter(table, &params[i]);
    for (int i = 0; i < stmt_count; i++)
        prescan_stmt(arena, stmts[i], table, -1);
    symbol_table_pop_scope(table);
}

void rust_project_body_facts(Arena *arena, Stmt **stmts, int count,
                             Parameter *params, int param_count, SymbolTable *table)
{
    prescan_function_body(arena, stmts, count, params, param_count, table);
}
...
{
    for (int i = 0; i < rust_g_thread_handle_var_count; i++)
        if (strcmp(rust_g_thread_handle_vars[i], name) == 0) return;
    if (rust_g_thread_handle_var_count % 8 == 0) {
        char **nv = arena_alloc(arena, (rust_g_thread_handle_var_count + 8) * sizeof(char *));
        for (int i = 0; i < rust_g_thread_handle_var_count; i++) nv[i] = rust_g_thread_handle_vars[i];
        rust_g_thread_handle_vars = nv;
    }
    rust_g_thread_handle_vars[rust_g_thread_handle_var_count++] = arena_strdup(arena, name);
}
...
{
    rust_g_thread_handle_vars = NULL;
    rust_g_thread_handle_var_count = 0;
    for (int i = 0; i < stmt_count; i++)
        prescan_thread_stmt(arena, stmts[i]);
}


json_object *rust_gen_model_function(Arena *arena, FunctionStmt *func, SymbolTable *symbol_table,
                                ArithmeticMode arithmetic_mode)
{
    ...
    if (rust_g_model_namespace_prefix != NULL)
    {
        ...
        else
        {
            ...
            snprintf(prefixed, sizeof(prefixed), "%s__%.*s",
                     rust_g_model_namespace_prefix, func->name.length, func->name.start);
    ...
    json_object_object_add(obj, "return_type", rust_gen_model_type(arena, func->return_type));
    json_object_object_add(obj, "modifier", json_object_new_string(rust_gen_model_func_mod_str(func->modifier)));
    ...
    if (func->name.filename)
        rust_gen_model_add_source_file(obj, func->name.filename);
    ...
    if (func->c_alias)
    {
        ...
        if (func->is_native && func->body_count == 0 && rust_g_has_pragma_source)
    ...
    else if (func->is_native || func->body_count > 0)
    {
        ...
        rust_g_has_pragma_source = false;
    }
    ...
    for (int i = 0; i < func->param_count; i++)
    {
        ...
        json_object_object_add(p, "type", rust_gen_model_type(arena, func->params[i].type));
        json_object_object_add(p, "mem_qual",
            json_object_new_string(rust_gen_model_mem_qual_str(func->params[i].mem_qualifier)));
    ...
    if (func->body)
    {
        rust_project_body_facts(arena, func->body, func->body_count,
                                func->params, func->param_count, symbol_table);
    ...
    rust_g_all_param_names = NULL;
    rust_g_all_param_count = 0;
    /* Reset for-each iter-var stack — populated as we descend into loop bodies. */
    rust_g_owned_iter_var_names = NULL;
    rust_g_owned_iter_var_count = 0;
    for (int i = 0; i < func->param_count; i++)
    {
        ...
        /* All params */
        if (rust_g_all_param_count % 8 == 0) {
            char **nv = arena_alloc(arena, (rust_g_all_param_count + 8) * sizeof(char *));
            for (int j = 0; j < rust_g_all_param_count; j++) nv[j] = rust_g_all_param_names[j];
            rust_g_all_param_names = nv;
        }
        rust_g_all_param_names[rust_g_all_param_count++] = ncopy;

        if (func->params[i].mem_qualifier == MEM_AS_REF)
        {
            /* Skip refcounted structs — 'as ref' on them is redundant
             * (already a pointer). The param is emitted as a single pointer
             * and field access via '->' is handled by the normal ref-struct
             * codepath, not by the as-ref-param deref path. */
            bool skip = (func->params[i].type &&
                         func->params[i].type->kind == TYPE_STRUCT &&
    ...
    rust_g_suppress_local_cleanup = false;
    ...
    if (func->body)
    {
        for (int i = 0; i < func->body_count; i++)
        {
            json_object_array_add(body,
                rust_gen_model_stmt(arena, func->body[i], symbol_table, arithmetic_mode));
        }
    }
    json_object_object_add(obj, "body", body);

    rust_g_in_main_void = was_in_main_void;
