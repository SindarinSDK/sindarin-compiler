...
static void code_gen_externs(CodeGen *gen)
{
    ...
    indented_fprintf(gen, 0, "extern char *rt_array_pop_string_h(RtManagedArena *, RtHandle);\n");
...
static void code_gen_native_extern_declaration(CodeGen *gen, FunctionStmt *fn)
{
    ...
    /* Native functions return raw C types, not RtHandle.
     * TYPE_STRING → char *, TYPE_ARRAY → element_type * */
    const char *ret_c;
    if (fn->return_type && fn->return_type->kind == TYPE_STRING) {
        ret_c = "char *";
        ...
        if (stmt->type == STMT_STRUCT_DECL)
        {
            ...
            for (int j = 0; j < struct_decl->method_count; j++)
            {
                ...
                if (method->is_native && method->body == NULL &&
                    ...
                    const char *elem_c = get_c_array_elem_type(gen->arena, method->return_type->as.array.element_type);
                    ret_type = arena_sprintf(gen->arena, "%s *", elem_c);
                } else {
