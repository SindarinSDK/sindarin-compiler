...
{
    RtHandleV2 *_h = rt_arena_v2_strdup(arena, val ? "true" : "false"); rt_handle_begin_transaction(_h); return (char *)_h->ptr;
}
...
{
    RtHandleV2 *_h = rt_arena_v2_strdup(arena, "void"); rt_handle_begin_transaction(_h); return (char *)_h->ptr;
}

char *rt_to_string_pointer(RtArenaV2 *arena, RtHandleV2 *p)
{
    if (p == NULL) {
        RtHandleV2 *_h = rt_arena_v2_strdup(arena, "nil"); rt_handle_begin_transaction(_h); return (char *)_h->ptr;
    }
    char buf[32];
