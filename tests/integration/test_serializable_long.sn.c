/*
 * Minimal JSON encoder/decoder for @serializable integration tests.
 * This is a test harness — the real implementation will live in the SDK.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
typedef struct {
    Buf *buf;       /* shared buffer */
    int first;      /* comma tracking */
    int is_array;   /* array vs object */
    int is_root;    /* true if this is the root encoder */
} JEnc;
...
/* Public factory: creates a new JSON encoder */
...
/*
 * Simple recursive-descent JSON parser for testing.
 * Parses a JSON string into a tree of JNode values.
 */
...
static JNode *jparse(const char **pp) {
    ...
    if (*p == '{') {
        ...
        /* Check if it's an integer (no dot, no e/E) */
        int is_int = 1;
...
/* Public factory: parse JSON string into a Decoder */
