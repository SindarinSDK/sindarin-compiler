/*
 * JSON encoder/decoder that sets __sn__cleanup on the Encoder.
 * This exercises the fix for the double-cleanup SEGV: encode() calls
 * Encoder.result() which finalizes ctx, then sn_auto_Encoder cleanup
 * calls __sn__cleanup.  Without the fix, __sn__cleanup dereferences
 * the NULL ctx and crashes.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/*
 * Cleanup callback — the critical piece.  Real encoder implementations
 * set this to free internal resources.  This version unconditionally
 * dereferences ctx, so if ctx is NULL (because result() already finalized
 * it), this will SEGV — exactly the bug we're testing the fix for.
 */
static void je_cleanup(__sn__Encoder *self) {
    JEnc *j = (JEnc *)self->__sn__ctx;
    /* Deliberately NO null-check — mimics real encoder behavior.
     * Before the fix this line would dereference NULL and crash. */
    if (j->buf) {
        free(j->buf->data);
        free(j->buf);
    }
...
static __sn__Encoder *je_make_sub(Buf *buf, int is_array) {
    ...
    enc->__sn__vt = &je_vt;
    enc->__sn__ctx = j;
    /* Sub-encoders don't get cleanup — parent owns the buffer */
    enc->__sn__cleanup = NULL;
    return enc;
}

/* Public factory: JSON object encoder with cleanup callback */
__sn__Encoder *cleanup_json_encoder(void) {
    __sn__Encoder *enc = calloc(1, sizeof(__sn__Encoder));
    JEnc *j = calloc(1, sizeof(JEnc));
    j->buf = buf_new();
    j->first = 1;
    ...
    enc->__sn__vt = &je_vt;
    enc->__sn__ctx = j;
    enc->__sn__cleanup = je_cleanup;  /* KEY: cleanup is set */
    return enc;
}

/* Public factory: JSON array encoder with cleanup callback */
__sn__Encoder *cleanup_json_array_encoder(void) {
    __sn__Encoder *enc = calloc(1, sizeof(__sn__Encoder));
    JEnc *j = calloc(1, sizeof(JEnc));
    j->buf = buf_new();
    j->first = 1;
...
static char *jd_read_str(__sn__Decoder *self, const char *key) {
    JNode *v = jnode_get(((void **)self->__sn__ctx)[0], key);
...
static long long jd_has_key(__sn__Decoder *self, const char *key) {
    return jnode_get(((void **)self->__sn__ctx)[0], key) != NULL;
}
...
static long long jd_length(__sn__Decoder *self) {
    JNode *n = ((void **)self->__sn__ctx)[0];
    if (n && n->type == JN_ARR) return n->arr.count;
    return 0;
}
static __sn__Decoder *jd_at(__sn__Decoder *self, long long index) {
    JNode *n = ((void **)self->__sn__ctx)[0];
    if (n && n->type == JN_ARR && index < n->arr.count)
        return jd_make(n->arr.items[index]);
    return jd_make(NULL);
}
static char *jd_at_str(__sn__Decoder *self, long long index) {
    JNode *n = ((void **)self->__sn__ctx)[0];
    if (n && n->type == JN_ARR && index < n->arr.count) {
        JNode *v = n->arr.items[index];
...
static double jd_at_double(__sn__Decoder *self, long long index) { (void)self; (void)index; return 0.0; }
static long long jd_at_bool(__sn__Decoder *self, long long index) { (void)self; (void)index; return 0; }

static __sn__DecoderVTable jd_vt = {
    .readStr = jd_read_str, .readInt = jd_read_int,
    .readDouble = jd_read_double, .readBool = jd_read_bool,
    .hasKey = jd_has_key, .readObject = jd_read_object,
    .readArray = jd_read_array, .length = jd_length,
    .at = jd_at, .atStr = jd_at_str,
    .atInt = jd_at_int, .atDouble = jd_at_double,
    .atBool = jd_at_bool,
};
...
static void jd_cleanup_root(__sn__Decoder *self) {
    void **ctx = (void **)self->__sn__ctx;
    if (ctx) jnode_free((JNode *)ctx[0]);
    free(ctx);
}

static __sn__Decoder *jd_make(JNode *node) {
    __sn__Decoder *dec = calloc(1, sizeof(__sn__Decoder));
    void **ctx = calloc(1, sizeof(void *));
    ctx[0] = node;
    dec->__sn__vt = &jd_vt;
    dec->__sn__ctx = ctx;
...
__sn__Decoder *cleanup_json_decoder(const char *input) {
    const char *p = input;
    JNode *root = jparse(&p);
