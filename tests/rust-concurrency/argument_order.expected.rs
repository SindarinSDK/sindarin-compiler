...
fn main() {
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x62, 0x65, 0x66, 0x6f, 0x72, 0x65, 0x20, 0x6a, 0x6f, 0x69, 0x6e, 0x3a, 0x20]))); __sn_interpolated.push_str(&format!("{}", { let value = __sn_concurrency0_global_sequence.lock().unwrap_or_else(|e| e.into_inner()).clone(); value })); __sn_interpolated }));
...
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x72, 0x65, 0x73, 0x75, 0x6c, 0x74, 0x3a, 0x20]))); __sn_interpolated.push_str(&format!("{}", result)); __sn_interpolated }));
    { if let Some(__sn_concurrency0_handle) = __sn_concurrency0_handle_result.take() { result = __sn_concurrency0_handle.join(); } result.clone() }
;
