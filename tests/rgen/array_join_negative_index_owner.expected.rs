...
impl __SnArrayText_0 for Bag {
    fn __sn_array_text_0(&self) -> SnString {
        let mut result = SnString::from_slice(b"Bag { ");
...
impl Holder {
    fn appendSeparator(&mut self) -> SnString {
        ((self).bags).push(Bag { values: vec![3, 4] });
        return SnString::from_slice(&[0x2d]);
    }
}

impl __SnArrayText_0 for Holder {
    fn __sn_array_text_0(&self) -> SnString {
        let mut result = SnString::from_slice(b"Holder { ");
        result.push_str("bags: ");
        result.push_str(&self.bags.__sn_struct_text_0());
        result.push_str(" }");
        result
    }
