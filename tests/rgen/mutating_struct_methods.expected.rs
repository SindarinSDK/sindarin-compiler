...
impl Counter {
    fn increment(&mut self) {
        ((self).value = __sn_checked_0(((self).value).checked_add(1), "Runtime error: integer overflow in addition"));
    }
    fn rename(&mut self, label: SnString) {
...
;
    ...
    fn describe(&self) -> SnString {
        return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&((self).label.clone())); __sn_interpolated.push_str("="); __sn_interpolated.push_str(&format!("{}", (self).value)); __sn_interpolated }
;
    }
}

fn main() {
    let mut counter: Counter = Counter { label: SnString::from("old"), value: 5 };
    let mut label: SnString = SnString::from("new");
...
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("source="); __sn_interpolated.push_str(&(label)); __sn_interpolated }
))
;
}
