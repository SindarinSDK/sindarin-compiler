...
impl Counter {
    ...
    fn describe(&self) -> SnString {
        return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&((self).label.clone())); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3d]))); __sn_interpolated.push_str(&format!("{}", (self).value)); __sn_interpolated };
    }
}
