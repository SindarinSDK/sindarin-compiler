...
fn check(calls: &mut i64) -> bool {
    __sn_println_string(&(SnString::from_slice(&[0x63, 0x6f, 0x6e, 0x64, 0x69, 0x74, 0x69, 0x6f, 0x6e])));
...
fn explain(calls: &mut i64) -> SnString {
    __sn_println_string(&(SnString::from_slice(&[0x6d, 0x65, 0x73, 0x73, 0x61, 0x67, 0x65])));
    { let __sn_place = &mut (*(calls)); let __sn_previous = *__sn_place; let __sn_next = __sn_checked_0(__sn_previous.checked_add(1), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_previous };
    return SnString::from_slice(&[0x62, 0x61, 0x64]);
}

fn main() {
    let mut calls: i64 = 0;
    { let __sn_assert_message = explain(&mut (calls)); let __sn_assert_condition = check(&mut (calls)); if !__sn_assert_condition { use std::io::Write as _; let mut __sn_assert_stderr = std::io::stderr().lock(); __sn_assert_stderr.write_all(__sn_assert_message.as_bytes()).expect("failed to write stderr"); __sn_assert_stderr.write_all(b"\n").expect("failed to write stderr"); std::process::exit(1); } }
;
}
