...
fn factory(offset: i64) -> __SnClosure<dyn Fn(i64) -> i64> {
    __sn_print_string(&(SnString::from_slice(&[0x66, 0x61, 0x63, 0x74, 0x6f, 0x72, 0x79, 0x0a])));
...
fn argument() -> i64 {
    __sn_print_string(&(SnString::from_slice(&[0x61, 0x72, 0x67, 0x75, 0x6d, 0x65, 0x6e, 0x74, 0x0a])));
    return 2;
}
...
fn main() {
    ...
    let mut immediate: __SnClosure<dyn Fn(i64) -> i64> = { self::__SnClosure::<dyn Fn(i64) -> i64>(std::rc::Rc::new(move |n: i64| -> i64 { __sn_checked_0((n).checked_add(1), "Runtime error: integer overflow in addition")})) }
;
