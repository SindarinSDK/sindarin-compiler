...
impl Metric {
    ...
    fn describe(&self, suffix: SnString) -> SnString {
        return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&((self).getName())); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3d]))); __sn_interpolated.push_str(&format!("{}", (self).doubled())); __sn_interpolated.push_str(&(suffix)); __sn_interpolated };
    }
}
