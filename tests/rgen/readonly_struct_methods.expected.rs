...
impl Metric {
    ...
    fn doubled(&self) -> i64 {
        return __sn_checked_0(((self).getValue()
 ).checked_mul(2), "Runtime error: integer overflow in multiplication");
    }
    fn getName(&self) -> SnString {
        return (self).name.clone();
    }
    fn describe(&self, suffix: SnString) -> SnString {
        return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&((self).getName()
 )); __sn_interpolated.push_str("="); __sn_interpolated.push_str(&format!("{}", (self).doubled()
 )); __sn_interpolated.push_str(&(suffix)); __sn_interpolated }
;
    }
}

fn main() {
    let mut metric: Metric = Metric { name: SnString::from("count"), value: 21 };
    let mut suffix: SnString = SnString::from("!");
    __sn_println_string(&((metric).describe(suffix.clone())
))
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("unchanged="); __sn_interpolated.push_str(&((metric).name)); __sn_interpolated.push_str("/"); __sn_interpolated.push_str(&(suffix)); __sn_interpolated }
))
;
}
