...
fn main() {
    ...
    {
    let mut __sn_iter_2 = (selectSource(&mut (evaluations))
).iter();
    while __sn_iter_2.hasNext() {
        let mut outer_value = __sn_iter_2.next();
        {
    let mut __sn_iter_1 = (selectSource(&mut (evaluations))
).iter();
    while __sn_iter_1.hasNext() {
        let mut inner_value = __sn_iter_1.next();
        __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("collision="); __sn_interpolated.push_str(&format!("{}", __sn_iter)); __sn_interpolated.push_str(" candidate="); __sn_interpolated.push_str(&format!("{}", __sn_iter_0)); __sn_interpolated.push_str(" outer="); __sn_interpolated.push_str(&format!("{}", outer_value)); __sn_interpolated.push_str(" inner="); __sn_interpolated.push_str(&format!("{}", inner_value)); __sn_interpolated }
))
;
        break;
    }
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("final="); __sn_interpolated.push_str(&format!("{}", __sn_iter)); __sn_interpolated.push_str(" candidate-final="); __sn_interpolated.push_str(&format!("{}", __sn_iter_0)); __sn_interpolated.push_str(" evaluations="); __sn_interpolated.push_str(&format!("{}", evaluations)); __sn_interpolated }
))
;
}
