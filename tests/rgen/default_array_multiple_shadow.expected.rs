...
fn mutate(first: &mut Vec<i64>, second: &mut Vec<i64>) -> i64 {
    if true {
        ...
        (second).push(10);
        __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x69, 0x6e, 0x6e, 0x65, 0x72, 0x3a]))); __sn_interpolated.push_str(&format!("{}", (second).len() as i64)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (second)[__sn_index((second).len(), 0)])); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (second)[__sn_index((second).len(), 1)])); __sn_interpolated }));
    }
    (first).push(2);
...
fn main() {
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x6f, 0x75, 0x74, 0x65, 0x72, 0x3a]))); __sn_interpolated.push_str(&format!("{}", (values).len() as i64)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (values)[__sn_index((values).len(), 0)])); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (values)[__sn_index((values).len(), 1)])); __sn_interpolated }));
}
