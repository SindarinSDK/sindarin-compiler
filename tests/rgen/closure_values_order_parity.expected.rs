...
;
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (({ let (__sn_functions, __sn_function_index) = (&(callbacks), selected); __sn_functions[__sn_index(__sn_functions.len(), __sn_function_index)].clone() }).0)(first.clone(), second.clone()))); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
...
;
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (((holder).action.clone()).0)(third.clone(), fourth.clone()))); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
