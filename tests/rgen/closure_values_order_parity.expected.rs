...
fn argument(value: i64) -> i64 {
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x61, 0x72, 0x67]))); __sn_interpolated.push_str(&format!("{}", value)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x0a]))); __sn_interpolated }));
    return value;
}

fn arraySource() -> Vec<__SnClosure<dyn Fn(i64, i64) -> i64>> {
    __sn_print_string(&(SnString::from_slice(&[0x61, 0x72, 0x72, 0x61, 0x79, 0x0a])));
...
fn fieldSource() -> Holder {
    __sn_print_string(&(SnString::from_slice(&[0x66, 0x69, 0x65, 0x6c, 0x64, 0x0a])));
...
fn index() -> i64 {
    __sn_print_string(&(SnString::from_slice(&[0x69, 0x6e, 0x64, 0x65, 0x78, 0x0a])));
    return 0;
}
...
fn recursive(n: i64) -> i64 {
    ...
    return __sn_checked_0((n).checked_mul(recursive(__sn_checked_0((n).checked_sub(1), "Runtime error: integer overflow in subtraction"))), "Runtime error: integer overflow in multiplication");
}
...
fn main() {
    ...
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (({ let (__sn_functions, __sn_function_index) = (&(callbacks), selected); __sn_functions[__sn_index(__sn_functions.len(), __sn_function_index)].clone() }).0)(first.clone(), second.clone()))); __sn_interpolated.push_str(&(SnString::from_slice(&[0x0a]))); __sn_interpolated }));
    let mut holder: Holder = fieldSource();
    let mut third: i64 = argument(3);
    let mut fourth: i64 = argument(4);
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (((holder).action.clone()).0)(third.clone(), fourth.clone()))); __sn_interpolated.push_str(&(SnString::from_slice(&[0x0a]))); __sn_interpolated }));
