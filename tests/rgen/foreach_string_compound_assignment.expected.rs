...
fn main() {
    ...
    for mut __sn_string_place in (names).iter().cloned() {
        ...
        { let __sn_rhs = 1; let __sn_place = &mut (index); let __sn_next = __sn_checked((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
    }
