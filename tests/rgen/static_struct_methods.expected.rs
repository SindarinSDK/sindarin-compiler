...
impl Label {
    fn wrap(value: SnString) -> Label {
        return Label { text: { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("["); __sn_interpolated.push_str(&(value)); __sn_interpolated.push_str("]"); __sn_interpolated }
  };
    }
}
...
fn main() {
    ...
    let mut source: SnString = SnString::from("hello");
    let mut label: Label = Label::wrap(source.clone());
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("point="); __sn_interpolated.push_str(&format!("{}", (point).x)); __sn_interpolated.push_str(","); __sn_interpolated.push_str(&format!("{}", (point).y)); __sn_interpolated }
))
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("origin="); __sn_interpolated.push_str(&format!("{}", (origin).x)); __sn_interpolated.push_str(","); __sn_interpolated.push_str(&format!("{}", (origin).y)); __sn_interpolated }
))
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("label="); __sn_interpolated.push_str(&((label).text)); __sn_interpolated.push_str("; source="); __sn_interpolated.push_str(&(source)); __sn_interpolated }
))
;
}
