...
impl Relay {
    fn relay(&mut self, values: &mut Vec<i64>, depth: i64) -> i64 {
        if (depth == 0) {
        (values).push(40);
        return (values).len() as i64;
    }
        if (depth == 2) {
        let mut values: Vec<i64> = vec![9];
        (values).push(10);
        __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x73, 0x68, 0x61, 0x64, 0x6f, 0x77, 0x3a]))); __sn_interpolated.push_str(&format!("{}", (values).len() as i64)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (values)[__sn_index((values).len(), 0)])); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (values)[__sn_index((values).len(), 1)])); __sn_interpolated }));
    }
        return { let __sn_array_call_arg_0 = depth; (self).step(&mut *(values), __sn_array_call_arg_0) };
    }
    fn step(&mut self, values: &mut Vec<i64>, depth: i64) -> i64 {
        ((self).values).push(depth);
        return { let __sn_array_call_arg_1 = __sn_checked_0((depth).checked_sub(1), "Runtime error: integer overflow in subtraction"); (self).relay(&mut *(values), __sn_array_call_arg_1) };
    }
    fn __sn_receiver_array_alias_0(&mut self, depth: i64) -> i64 {
        if (depth == 0) {
        ((self).values).push(40);
        ...
        return (self).__sn_receiver_array_alias_1(depth);
    }
    fn __sn_receiver_array_alias_1(&mut self, depth: i64) -> i64 {
        ((self).values).push(depth);
        return (self).__sn_receiver_array_alias_0(__sn_checked_0((depth).checked_sub(1), "Runtime error: integer overflow in subtraction"));
    }
}
