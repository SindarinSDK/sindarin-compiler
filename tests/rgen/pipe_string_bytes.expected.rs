...
fn makeBinary(calls: &mut i64) -> SnString {
    ...
    return SnString::from_slice(&[0x61, 0xaf, 0x0d, 0x0a, 0x62, 0x80, 0x0a]);
}
...
;
    let mut first: Vec<u8> = __sn_string_to_bytes(&((lines)[__sn_index((lines).len(), 0)]))
;
    let mut second: Vec<u8> = __sn_string_to_bytes(&((lines)[__sn_index((lines).len(), 1)]))
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", calls)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (lines).len() as i64)); __sn_interpolated }));
