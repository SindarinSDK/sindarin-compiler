...
fn main() {
    ...
    match (nextSubject(&mut (subject_calls)) as i64) {
        1 | 2 => {
            (selected = 10);
            { let __sn_place = &mut (effects); let __sn_previous = *__sn_place; let __sn_next = __sn_checked(__sn_previous.checked_add(1), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_previous };
        },
        2 | 3 => {
            (selected = 20);
            { let __sn_rhs = 100; let __sn_place = &mut (effects); let __sn_next = __sn_checked((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
        },
        _ => {
            (selected = 30);
            { let __sn_rhs = 1000; let __sn_place = &mut (effects); let __sn_next = __sn_checked((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
        },
    };
    ...
    match ((-9223372036854775807) as i64) {
        (-9223372036854775807) | 9223372036854775807 => {
            { let __sn_rhs = 1; let __sn_place = &mut (parser_extremes); let __sn_next = __sn_checked((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
        },
        _ => {
            (parser_extremes = 100);
        },
    ...
    match (9223372036854775807 as i64) {
        (-9223372036854775807) | 9223372036854775807 => {
            { let __sn_rhs = 10; let __sn_place = &mut (parser_extremes); let __sn_next = __sn_checked((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
        },
        _ => {
            (parser_extremes = 200);
        },
