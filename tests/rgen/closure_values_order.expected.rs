...
fn argument(value: i64) -> i64 {
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("arg"); __sn_interpolated.push_str(&format!("{}", value)); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
    return value;
}
...
fn arraySource() -> Vec<__SnClosure<dyn Fn(i64, i64) -> i64>> {
    __sn_print_string(&(SnString::from("array\n")))
;
...
fn fieldSource() -> Holder {
    __sn_print_string(&(SnString::from("field\n")))
;
...
fn index() -> i64 {
    __sn_print_string(&(SnString::from("index\n")))
;
    return 0;
}
...
fn recursive(n: i64) -> i64 {
    ...
    return __sn_checked_0((n).checked_mul(recursive(__sn_checked_0((n).checked_sub(1), "Runtime error: integer overflow in subtraction"))
 ), "Runtime error: integer overflow in multiplication");
}

fn main() {
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (({ let (__sn_functions, __sn_function_index) = (&(arraySource()
), index()
); __sn_functions[__sn_index(__sn_functions.len(), __sn_function_index)].clone() }).0)(argument(1)
, argument(2)
))); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (((fieldSource()
).action.clone()).0)(argument(3)
, argument(4)
))); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
...
;
    ...
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", ((copy.clone()).0)(5))); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", ((factorial.clone()).0)(4))); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
}
