...
fn main() {
    print!("{}", { let mut __sn_interpolated = String::new(); __sn_interpolated.push_str(&format!("{}", (({ let (__sn_functions, __sn_function_index) = (&(arraySource()), index()); __sn_functions[__sn_index(__sn_functions.len(), __sn_function_index)].clone() }).0)(argument(1), argument(2)))); __sn_interpolated.push_str("\n"); __sn_interpolated });
    print!("{}", { let mut __sn_interpolated = String::new(); __sn_interpolated.push_str(&format!("{}", (((fieldSource()).action.clone()).0)(argument(3), argument(4)))); __sn_interpolated.push_str("\n"); __sn_interpolated });
