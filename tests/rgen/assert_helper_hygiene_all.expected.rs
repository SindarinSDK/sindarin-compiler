...
fn main() {
    ...
    { let __sn_assert_condition_1 = __sn_assert_condition; let __sn_assert_message_1 = __sn_assert_message; if !__sn_assert_condition_1 { use std::io::Write as _; let mut __sn_assert_stderr_1 = std::io::stderr().lock(); __sn_assert_stderr_1.write_all(__sn_assert_message_1.as_bytes()).expect("failed to write stderr"); __sn_assert_stderr_1.write_all(b"\n").expect("failed to write stderr"); std::process::exit(1); } }
;
