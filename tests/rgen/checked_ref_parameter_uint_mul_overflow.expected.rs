...
fn fail(value: &mut u64) {
    let mut two: u64 = 2;
    { let (__sn_byte_rhs, __sn_byte_place): (u64, &mut u64) = (two, &mut (*(value))); let __sn_byte_next = (*__sn_byte_place).wrapping_mul(__sn_byte_rhs); *__sn_byte_place = __sn_byte_next; __sn_byte_next };
}
