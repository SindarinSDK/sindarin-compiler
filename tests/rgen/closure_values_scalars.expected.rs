...
;
    let mut fi32: __SnClosure<dyn Fn(i32) -> i32> = { let (i32, ) = (i32.clone(), ); self::__SnClosure::<dyn Fn(i32) -> i32>(std::rc::Rc::new(move |x: i32| -> i32 { { let (__sn_byte_left, __sn_byte_right): (i32, i32) = (i32.clone(), x); __sn_byte_left.wrapping_add(__sn_byte_right) }})) }
;
    let mut fb: __SnClosure<dyn Fn(u8) -> u8> = { let (b, ) = (b.clone(), ); self::__SnClosure::<dyn Fn(u8) -> u8>(std::rc::Rc::new(move |x: u8| -> u8 { { let (__sn_byte_left, __sn_byte_right): (u8, u8) = (b.clone(), x); __sn_byte_left.wrapping_add(__sn_byte_right) }})) }
;
    let mut fu32: __SnClosure<dyn Fn(u32) -> u32> = { let (u32, ) = (u32.clone(), ); self::__SnClosure::<dyn Fn(u32) -> u32>(std::rc::Rc::new(move |x: u32| -> u32 { { let (__sn_byte_left, __sn_byte_right): (u32, u32) = (u32.clone(), x); __sn_byte_left.wrapping_add(__sn_byte_right) }})) }
;
    let mut fu: __SnClosure<dyn Fn(u64) -> u64> = { let (u, ) = (u.clone(), ); self::__SnClosure::<dyn Fn(u64) -> u64>(std::rc::Rc::new(move |x: u64| -> u64 { { let (__sn_byte_left, __sn_byte_right): (u64, u64) = (u.clone(), x); __sn_byte_left.wrapping_add(__sn_byte_right) }})) }
;
...
;
    ...
    (yes = false);
    (letter = '\u{5a}');
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", ((fi.clone()).0)(1))); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", ((fl.clone()).0)(1))); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", ((fi32.clone()).0)(1))); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", ((fb.clone()).0)(1))); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", ((fu32.clone()).0)(1))); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", ((fu.clone()).0)(1))); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{:.5}", ((ff.clone()).0)(0.5))); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{:.5}", ((fd.clone()).0)(0.5))); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", ((fy.clone()).0)(true))); __sn_interpolated.push_str(":"); __sn_interpolated.push_char(((fc.clone()).0)('\u{58}')); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
...
;
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", ((many.clone()).0)(1, 2, 3, 4, 5, 6, 7.0, 8.0, true, '\u{4e}'))); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
    let mut empty: __SnClosure<dyn Fn() -> ()> = { self::__SnClosure::<dyn Fn() -> ()>(std::rc::Rc::new(move || -> () { __sn_print_string(&(SnString::from("void\n")))
;})) }
;
    ((empty.clone()).0)();
}
