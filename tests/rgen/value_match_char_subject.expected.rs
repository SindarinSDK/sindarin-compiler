...
fn observe(calls: &mut i64, value: char) -> char {
...
fn main() {
    let mut calls: i64 = 0;
    let mut selected: i64 = match (observe(&mut (calls), '\u{ff}') as char) {
        '\u{61}' => {
            (0 as i64)
        },
        '\u{ff}' => {
    ...
    let mut result: char = match ('\u{ff}' as char) {
        '\u{ff}' => {
            ('\u{7a}' as char)
        },
        _ => {
