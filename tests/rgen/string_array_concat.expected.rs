...
fn main() {
    let mut left: Vec<SnString> = vec![SnString::from("alpha"), SnString::from("beta")];
    let mut right: Vec<SnString> = vec![SnString::from("gamma"), SnString::from("delta")];
...
;
    __sn_println_string(&({ let __sn_array = &(combined); let __sn_separator = &(SnString::from(",")); __sn_string_join(__sn_array, __sn_separator) }

))
;
    { let __sn_array_index = __sn_index((left).len(), 0); (left)[__sn_array_index] = SnString::from("changed-left"); };
    { let __sn_array_index = __sn_index((right).len(), 1); (right)[__sn_array_index] = SnString::from("changed-right"); };
    __sn_println_string(&({ let __sn_array = &(left); let __sn_separator = &(SnString::from(",")); __sn_string_join(__sn_array, __sn_separator) }

))
;
    __sn_println_string(&({ let __sn_array = &(right); let __sn_separator = &(SnString::from(",")); __sn_string_join(__sn_array, __sn_separator) }
