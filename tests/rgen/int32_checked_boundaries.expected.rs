...
fn main() {
    ...
    let mut one: i32 = 1;
    let mut sum: i32 = __sn_checked_0((add_base).checked_add(one), "Runtime error: integer overflow in addition");
    ...
    let mut two: i32 = 2;
    let mut product: i32 = __sn_checked_0((mul_base).checked_mul(two), "Runtime error: integer overflow in multiplication");
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", sum)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", minimum)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", product)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", quotient)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", remainder)); __sn_interpolated }
))
;
}
