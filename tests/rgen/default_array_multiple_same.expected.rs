...
fn __sn_array_alias_call_0() {
    __sn_println_string(&(SnString::from_slice(&[0x73, 0x6f, 0x75, 0x72, 0x63, 0x65, 0x20, 0x68, 0x65, 0x6c, 0x70, 0x65, 0x72])));
}
...
fn mark(calls: &mut i64) -> i64 {
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x6d, 0x61, 0x72, 0x6b, 0x3a]))); __sn_interpolated.push_str(&format!("{}", *(calls))); __sn_interpolated }));
    return 4;
}
...
fn main() {
    ...
    println!("{}", calls);
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (shared).len() as i64)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (shared)[__sn_index((shared).len(), 1)])); __sn_interpolated }));
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (distinct).len() as i64)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (distinct)[__sn_index((distinct).len(), 1)])); __sn_interpolated }));
