...
fn main() {
    ...
    let mut expression_sizes: i64 = __sn_checked_0((__sn_checked_0((8i64).checked_add(4i64), "Runtime error: integer overflow in addition")).checked_add(1i64), "Runtime error: integer overflow in addition");
    let mut arithmetic: i64 = __sn_checked_0((8i64).checked_add(__sn_checked_0((4i64).checked_mul(1i64), "Runtime error: integer overflow in multiplication")), "Runtime error: integer overflow in addition");
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", types_ok)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", expressions_ok)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", ((arithmetic == 12) && comparison))); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", (counter == 0))); __sn_interpolated }
))
;
}
