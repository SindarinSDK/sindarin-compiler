...
fn __sn_byte_array_to_hex(values: &[u8]) -> SnString {
    const HEX: &[u8; 16] = b"0123456789abcdef";
    let mut result = Vec::with_capacity(values.len().saturating_mul(2));
    for value in values {
        result.push(HEX[(value >> 4) as usize]);
        result.push(HEX[(value & 0x0f) as usize]);
    }
    SnString::from_bytes(result)
}
...
fn produce(calls: &mut i64) -> Vec<u8> {
...
fn main() {
    let mut __sn_array: Vec<u8> = vec![0, 1, 15, 16, 171, 255];
    __sn_println_string(&({ let __sn_array_0 = &(__sn_array); __sn_byte_array_to_hex(__sn_array_0.as_slice()) }));
    (__sn_array).push(0);
    (__sn_array).push(15);
    __sn_println_string(&({ let __sn_array_0 = &(__sn_array); __sn_byte_array_to_hex(__sn_array_0.as_slice()) }));
    let mut empty: Vec<u8> = vec![];
    __sn_print_string(&(SnString::from_slice(&[0x65, 0x6d, 0x70, 0x74, 0x79, 0x3d, 0x3c])));
    __sn_print_string(&({ let __sn_array_0 = &(empty); __sn_byte_array_to_hex(__sn_array_0.as_slice()) }));
    __sn_println_string(&(SnString::from_slice(&[0x3e])));
    let mut calls: i64 = 0;
    __sn_println_string(&({ let __sn_array_0 = &(produce(&mut (calls))); __sn_byte_array_to_hex(__sn_array_0.as_slice()) }));
    println!("{}", calls);
}
