...
;
    ...
    ((item).value = 9);
    __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (result).value)); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", (item).value)); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", __SnClosure_1)); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
}
