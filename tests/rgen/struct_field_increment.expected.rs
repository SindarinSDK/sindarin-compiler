...
impl Counter {
    fn next(&mut self) -> i64 {
        return { let __sn_place = &mut ((self).value); let __sn_previous = *__sn_place; let __sn_next = __sn_checked(__sn_previous.checked_add(1), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_previous };
    }
    fn previous(&mut self) -> i64 {
        return { let __sn_place = &mut ((self).value); let __sn_previous = *__sn_place; let __sn_next = __sn_checked(__sn_previous.checked_sub(1), "Runtime error: integer overflow in subtraction"); *__sn_place = __sn_next; __sn_previous };
    }
