...
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("increment="); __sn_interpolated.push_str(&format!("{}", beforeIncrement)); __sn_interpolated.push_str("->"); __sn_interpolated.push_str(&format!("{}", (counter).value)); __sn_interpolated }
))
;
    let mut beforeDecrement: i64 = (counter).previous()
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("decrement="); __sn_interpolated.push_str(&format!("{}", beforeDecrement)); __sn_interpolated.push_str("->"); __sn_interpolated.push_str(&format!("{}", (counter).value)); __sn_interpolated }
))
;
}
