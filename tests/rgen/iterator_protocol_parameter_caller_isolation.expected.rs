...
fn mutate(mut parameter: i64) -> i64 {
    let mut source: IntIterator = IntIterator { remaining: 2 };
    {
    ...
    while __sn_iter_0.hasNext() {
        ...
        { let (__sn_rhs, __sn_place): (i64, &mut i64) = (value, &mut (parameter)); let __sn_next = *__sn_place + __sn_rhs; *__sn_place = __sn_next; __sn_next };
    }
}
    return parameter;
}
