...
fn main() {
    let mut field: FieldInfo = FieldInfo { name: SnString::from("count"), typeName: SnString::from("int"), typeId: 367623774 };
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&((field).name)); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&((field).typeName)); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", (field).typeId)); __sn_interpolated }
))
;
}
