...
fn main() {
    ...
    let mut one: i64 = 1;
    { let __sn_rhs = one; let __sn_place = &mut (max); let __sn_next = __sn_checked((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
}
