...
impl Counter {
    fn localMath(&self) -> i64 {
        let mut total: i64 = 4;
        { let __sn_rhs = 3; let __sn_place = &mut (total); let __sn_next = __sn_checked((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
        return total;
    }
}
