...
fn main() {
    ...
    println!("{}", (point).y);
    println!("{}", (copied).y);
    let mut label: Label = Label { text: SnString::from_slice(&[0x64, 0x65, 0x66, 0x61, 0x75, 0x6c, 0x74]) };
    let mut label_copy: Label = label.clone();
    ((label_copy).text = SnString::from_slice(&[0x63, 0x68, 0x61, 0x6e, 0x67, 0x65, 0x64]));
    __sn_println_string(&((label).text));
    __sn_println_string(&((label_copy).text));
    let mut marker: Marker = Marker { point: point, label: label.clone() };
    (((marker).label).text = SnString::from_slice(&[0x6e, 0x65, 0x73, 0x74, 0x65, 0x64]));
