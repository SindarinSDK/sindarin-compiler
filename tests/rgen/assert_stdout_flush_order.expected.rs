...
fn main() {
    __sn_println_string(&(SnString::from_slice(&[0x54, 0x65, 0x73, 0x74, 0x69, 0x6e, 0x67, 0x20, 0x61, 0x73, 0x73, 0x65, 0x72, 0x74])));
    { let __sn_assert_condition = true; let __sn_assert_message = SnString::from_slice(&[0x54, 0x68, 0x69, 0x73, 0x20, 0x73, 0x68, 0x6f, 0x75, 0x6c, 0x64, 0x20, 0x6e, 0x6f, 0x74, 0x20, 0x66, 0x61, 0x69, 0x6c]); if !__sn_assert_condition { use std::io::Write as _; let mut __sn_assert_stderr = std::io::stderr().lock(); __sn_assert_stderr.write_all(__sn_assert_message.as_bytes()).expect("failed to write stderr"); __sn_assert_stderr.write_all(b"\n").expect("failed to write stderr"); std::process::exit(1); } }
;
    __sn_println_string(&(SnString::from_slice(&[0x46, 0x69, 0x72, 0x73, 0x74, 0x20, 0x61, 0x73, 0x73, 0x65, 0x72, 0x74, 0x20, 0x70, 0x61, 0x73, 0x73, 0x65, 0x64])));
    { let __sn_assert_condition_1 = ({ let (__sn_left, __sn_right): (i64, i64) = (1, 1); __sn_checked_0(__sn_left.checked_add(__sn_right), "Runtime error: integer overflow in addition") } == 2); let __sn_assert_message_1 = SnString::from_slice(&[0x4d, 0x61, 0x74, 0x68, 0x20, 0x77, 0x6f, 0x72, 0x6b, 0x73]); if !__sn_assert_condition_1 { use std::io::Write as _; let mut __sn_assert_stderr_1 = std::io::stderr().lock(); __sn_assert_stderr_1.write_all(__sn_assert_message_1.as_bytes()).expect("failed to write stderr"); __sn_assert_stderr_1.write_all(b"\n").expect("failed to write stderr"); std::process::exit(1); } }
;
    __sn_println_string(&(SnString::from_slice(&[0x53, 0x65, 0x63, 0x6f, 0x6e, 0x64, 0x20, 0x61, 0x73, 0x73, 0x65, 0x72, 0x74, 0x20, 0x70, 0x61, 0x73, 0x73, 0x65, 0x64])));
    { let __sn_assert_condition_2 = false; let __sn_assert_message_2 = SnString::from_slice(&[0x41, 0x73, 0x73, 0x65, 0x72, 0x74, 0x69, 0x6f, 0x6e, 0x20, 0x66, 0x61, 0x69, 0x6c, 0x65, 0x64, 0x3a, 0x20, 0x65, 0x78, 0x70, 0x65, 0x63, 0x74, 0x65, 0x64, 0x20, 0x63, 0x6f, 0x6e, 0x64, 0x69, 0x74, 0x69, 0x6f, 0x6e, 0x20, 0x74, 0x6f, 0x20, 0x62, 0x65, 0x20, 0x74, 0x72, 0x75, 0x65]); if !__sn_assert_condition_2 { use std::io::Write as _; let mut __sn_assert_stderr_2 = std::io::stderr().lock(); __sn_assert_stderr_2.write_all(__sn_assert_message_2.as_bytes()).expect("failed to write stderr"); __sn_assert_stderr_2.write_all(b"\n").expect("failed to write stderr"); std::process::exit(1); } }
;
    __sn_println_string(&(SnString::from_slice(&[0x54, 0x68, 0x69, 0x73, 0x20, 0x73, 0x68, 0x6f, 0x75, 0x6c, 0x64, 0x20, 0x6e, 0x6f, 0x74, 0x20, 0x62, 0x65, 0x20, 0x70, 0x72, 0x69, 0x6e, 0x74, 0x65, 0x64])));
}
