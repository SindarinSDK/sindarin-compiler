...
fn main() {
    ...
    for mut value in (values).iter().cloned() {
        let mut added: i64 = { let __sn_rhs = rhs(&mut (calls)); let __sn_place = &mut (value); let __sn_next = __sn_checked_0((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
        let mut divided: i64 = { let __sn_rhs = rhs(&mut (calls)); let __sn_place = &mut (value); let __sn_next = __sn_checked_div_0((*__sn_place).checked_div(__sn_rhs), __sn_rhs == 0); *__sn_place = __sn_next; __sn_next };
