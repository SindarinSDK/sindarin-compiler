...
impl Formatter {
    fn bracket(value: SnString) -> SnString {
...
;
    ...
    fn describe(&self, suffix: SnString) -> SnString {
        return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(Formatter::bracket((self).prefix.clone()))); __sn_interpolated.push_str(&(suffix)); __sn_interpolated }
;
    }
}
#[derive(Clone, Debug, PartialEq)]
...
fn main() {
    let mut formatter: Formatter = Formatter { prefix: SnString::from("item") };
    let mut suffix: SnString = SnString::from("!");
    let mut label: Label = Label { text: SnString::from("source") };
    let mut copied: Label = (label).copy()
;
    __sn_println_string(&((formatter).describe(suffix.clone())
))
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("copy="); __sn_interpolated.push_str(&((copied).text)); __sn_interpolated.push_str("; original="); __sn_interpolated.push_str(&((label).text)); __sn_interpolated.push_str("; suffix="); __sn_interpolated.push_str(&(suffix)); __sn_interpolated }
))
;
}
