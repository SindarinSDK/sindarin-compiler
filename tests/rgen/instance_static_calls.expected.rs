...
impl Formatter {
    fn bracket(value: SnString) -> SnString {
        return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x5b]))); __sn_interpolated.push_str(&(value)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x5d]))); __sn_interpolated };
    }
