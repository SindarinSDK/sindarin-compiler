...
fn main() {
    ...
    println!("{}", (((((single_add == 20.0)
 && (single_subtract == 18.0)
)
 && (single_multiply == 9.0)
)
 && (single_divide == 3.0)
 ...
 && (single == 3.0)
    ...
    println!("{}", (((((precise_add == 40.0)
 && (precise_subtract == 36.0)
)
 && (precise_multiply == 18.0)
)
 && (precise_divide == 6.0)
)
 && (precise == 6.0)
 ...
 && ((singles)[__sn_index((singles).len(), single_index)] == 8.0)
)
);
        { let __sn_rhs = 1; let __sn_place = &mut (single_index); let __sn_next = __sn_checked((*__sn_place).checked_add(__sn_rhs), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_next };
    }
 ...
 && ((doubles)[__sn_index((doubles).len(), 0)] == 16.0)
)
);
    }
    ...
    println!("{}", ((infinity_result == infinity)
 && (infinity > 1.0)
)
);
}
