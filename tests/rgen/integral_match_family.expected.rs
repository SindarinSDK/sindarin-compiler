...
fn main() {
    ...
    match (observeLong(&mut (subject_calls), &mut (order), 2) as i64) {
        ...
        2 => {
            (selected = 20);
            (order = ((order).checked_mul(10).expect("checked arithmetic failed")).checked_add(8).expect("checked arithmetic failed"));
        },
        _ => {
            (selected = 30);
            (order = ((order).checked_mul(10).expect("checked arithmetic failed")).checked_add(9).expect("checked arithmetic failed"));
        },
    };
    ...
    match ((-9223372036854775807) as i64) {
        (-9223372036854775807) => {
            { let __sn_place = &mut (boundary_hits); let __sn_previous = *__sn_place; let __sn_next = __sn_previous.checked_add(1).expect("checked arithmetic failed"); *__sn_place = __sn_next; __sn_previous };
        },
        _ => {},
    };
    match (9223372036854775807 as i64) {
    ...
    { let __sn_rhs = 1; let __sn_place = &mut (int32_min); let __sn_next = (*__sn_place).checked_sub(__sn_rhs).expect("checked arithmetic failed"); *__sn_place = __sn_next; __sn_next };
