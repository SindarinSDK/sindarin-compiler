...
fn main() {
    let mut ascii: SnString = SnString::from("ASCII");
    let mut accent: SnString = SnString::from("é");
    let mut world: SnString = SnString::from("世界");
    let mut emoji: SnString = SnString::from("🙂");
    let mut decomposed: SnString = SnString::from("é");
    let mut controls: SnString = SnString::from("\n\t\r\u{1f}");
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", (ascii).len() as i64)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", (accent).len() as i64)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", (world).len() as i64)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", (emoji).len() as i64)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", (decomposed).len() as i64)); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", (controls).len() as i64)); __sn_interpolated }
))
;
}
