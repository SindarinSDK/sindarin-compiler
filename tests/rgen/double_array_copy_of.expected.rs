...
fn main() {
    ...
    let mut negative_infinity: f64 = ((-1.0) / zero);
    let mut source: Vec<f64> = vec![1.25, negative_zero, nan, infinity, negative_infinity];
    let mut copied: Vec<f64> = (source).clone();
...
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{:.2}", (copied)[__sn_index((copied).len(), 0)])); __sn_interpolated }
))
;
...
;
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{:.2}", (source)[__sn_index((source).len(), 0)])); __sn_interpolated.push_str(","); __sn_interpolated.push_str(&format!("{:.2}", (source)[__sn_index((source).len(), 4)])); __sn_interpolated }
))
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{:.2}", (copied)[__sn_index((copied).len(), 0)])); __sn_interpolated.push_str(","); __sn_interpolated.push_str(&format!("{:.2}", (copied)[__sn_index((copied).len(), 4)])); __sn_interpolated }
))
;
}
