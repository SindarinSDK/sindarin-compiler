...
fn main() {
    let mut pattern: SnString = SnString::from("one");
    {
    let __sn_match_subject_0: SnString = SnString::from("one");
    if (__sn_match_subject_0 == (pattern)) {
        __sn_println_string(&(SnString::from("matched")))
;
    }
    else {
        __sn_println_string(&(SnString::from("other")))
;
    }
};
}
