...
fn factory(offset: i64) -> __SnClosure<dyn Fn(i64) -> i64> {
    return { let (offset, ) = (offset.clone(), ); self::__SnClosure::<dyn Fn(i64) -> i64>(std::rc::Rc::new(move |x: i64| -> i64 { __sn_checked_0((x).checked_add(offset.clone()), "Runtime error: integer overflow in addition")})) }
;
}
