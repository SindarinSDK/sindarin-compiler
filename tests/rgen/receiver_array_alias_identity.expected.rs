...
impl Bag {
    fn __sn_receiver_array_alias_0(&self) -> i64 {
        return 77;
    }
    fn mutateBoth(&mut self, values: &mut Vec<i64>) -> i64 {
        if true {
        let mut values: Vec<i64> = vec![9];
        (values).push(10);
        __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x69, 0x6e, 0x6e, 0x65, 0x72, 0x3a]))); __sn_interpolated.push_str(&format!("{}", (values).len() as i64)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (values)[__sn_index((values).len(), 0)])); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&format!("{}", (values)[__sn_index((values).len(), 1)])); __sn_interpolated }));
    }
        ((self).values).push(2);
        (values).push(((self).values.clone()).len() as i64);
        return __sn_checked_0((__sn_checked_0((((self).values.clone()).len() as i64).checked_mul(10), "Runtime error: integer overflow in multiplication")).checked_add((values).len() as i64), "Runtime error: integer overflow in addition");
    }
    fn __sn_receiver_array_alias_1(&mut self) -> i64 {
        ...
        ((self).values).push(2);
        ((self).values).push(((self).values.clone()).len() as i64);
        return __sn_checked_0((__sn_checked_0((((self).values.clone()).len() as i64).checked_mul(10), "Runtime error: integer overflow in multiplication")).checked_add(((self).values).len() as i64), "Runtime error: integer overflow in addition");
    }
}
...
fn main() {
    ...
    println!("{}", (bag).mutateBoth(&mut (other)));
