...
impl Receiver {
    fn takeStatic(value: f32) -> f32 {
        return value;
    }
    fn takeInstance(&self, value: f32) -> f32 {
        return value;
    }
}
...
fn main() {
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{:.5}", Receiver::takeStatic((((((nextFloat(&mut (trace), 1, 1.25)) as f64) * ((2.0) as f64))) as f32)))); __sn_interpolated }));
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{:.5}", (receiver).takeInstance((((((nextFloat(&mut (trace), 2, 1.25)) as f64) * ((3.0) as f64))) as f32)))); __sn_interpolated }));
    println!("{}", trace);
}
