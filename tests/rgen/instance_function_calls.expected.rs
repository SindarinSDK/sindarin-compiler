...
impl Item {
    fn describe(&self, suffix: SnString) -> SnString {
...
fn decorate(value: SnString, suffix: SnString) -> SnString {
    return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("<"); __sn_interpolated.push_str(&(value)); __sn_interpolated.push_str(">"); __sn_interpolated.push_str(&(suffix)); __sn_interpolated }
;
}

fn makeLabel(value: SnString) -> Label {
    return Label { text: value.clone() };
}

fn main() {
    let mut item: Item = Item { name: SnString::from("source") };
    let mut suffix: SnString = SnString::from("!");
    let mut label: Label = (item).copyLabel()
;
    __sn_println_string(&((item).describe(suffix.clone())
))
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("copy="); __sn_interpolated.push_str(&((label).text)); __sn_interpolated.push_str("; original="); __sn_interpolated.push_str(&((item).name)); __sn_interpolated.push_str("; suffix="); __sn_interpolated.push_str(&(suffix)); __sn_interpolated }
))
;
}
