...
fn decorate(value: SnString, suffix: SnString) -> SnString {
    return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3c]))); __sn_interpolated.push_str(&(value)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3e]))); __sn_interpolated.push_str(&(suffix)); __sn_interpolated };
}
...
fn main() {
    let mut item: Item = Item { name: SnString::from_slice(&[0x73, 0x6f, 0x75, 0x72, 0x63, 0x65]) };
