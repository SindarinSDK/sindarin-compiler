...
fn main() {
    ...
    __sn_println_string(&({ let __sn_separator_0 = &(SnString::from_slice(&[])); __sn_string_join((words).as_slice(), __sn_separator_0) }));
    let mut single: Vec<SnString> = vec![SnString::from_slice(&[0x61, 0x6c, 0x6f, 0x6e, 0x65])];
