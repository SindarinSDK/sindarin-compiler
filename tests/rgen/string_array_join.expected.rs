...
fn main() {
    let mut words: Vec<SnString> = vec![SnString::from("the"), SnString::from("quick"), SnString::from("fox")];
    let mut separator: SnString = SnString::from(" | ");
    __sn_println_string(&({ let __sn_array = &(words); let __sn_separator = &(separator); __sn_string_join(__sn_array, __sn_separator) }

))
;
    __sn_println_string(&(separator))
;
    __sn_println_string(&({ let __sn_array = &(words); let __sn_separator = &(SnString::from("")); __sn_string_join(__sn_array, __sn_separator) }

))
;
    let mut single: Vec<SnString> = vec![SnString::from("alone")];
    __sn_println_string(&({ let __sn_array = &(single); let __sn_separator = &(SnString::from(",")); __sn_string_join(__sn_array, __sn_separator) }

))
;
    let mut empty: Vec<SnString> = vec![];
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("empty=|"); __sn_interpolated.push_str(&({ let __sn_array = &(empty); let __sn_separator = &(SnString::from(",")); __sn_string_join(__sn_array, __sn_separator) }

)); __sn_interpolated.push_str("|"); __sn_interpolated }
))
;
}
