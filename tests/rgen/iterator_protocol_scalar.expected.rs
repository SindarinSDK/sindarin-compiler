...
fn main() {
    ...
    let mut sum: i64 = 0;
    {
    let mut __sn_iter_0 = ((sources)[__sn_index((sources).len(), selectSource(&mut (evaluations))
)]).iter();
    while __sn_iter_0.hasNext() {
        let mut value = __sn_iter_0.next();
        let mut produced: i64 = value;
        (value = (-1));
        __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("value="); __sn_interpolated.push_str(&format!("{}", produced)); __sn_interpolated.push_str(" binding="); __sn_interpolated.push_str(&format!("{}", value)); __sn_interpolated }
))
;
        if (produced == 218) {
        continue;
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("natural evaluations="); __sn_interpolated.push_str(&format!("{}", evaluations)); __sn_interpolated.push_str(" sum="); __sn_interpolated.push_str(&format!("{}", sum)); __sn_interpolated }
))
;
    {
    let mut __sn_iter_1 = ((sources)[__sn_index((sources).len(), selectSource(&mut (evaluations))
)]).iter();
    while __sn_iter_1.hasNext() {
        ...
        __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("before-break="); __sn_interpolated.push_str(&format!("{}", value)); __sn_interpolated }
))
;
    }
}
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("total evaluations="); __sn_interpolated.push_str(&format!("{}", evaluations)); __sn_interpolated.push_str(" source="); __sn_interpolated.push_str(&format!("{}", ((sources)[__sn_index((sources).len(), 0)]).start)); __sn_interpolated }
))
;
    let mut outer: i64 = 0;
    {
        ...
        while (outer < 1) {
            {
    let mut __sn_iter_2 = ((sources)[__sn_index((sources).len(), selectSource(&mut (evaluations))
)]).iter();
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str("nested outer="); __sn_interpolated.push_str(&format!("{}", outer)); __sn_interpolated.push_str(" evaluations="); __sn_interpolated.push_str(&format!("{}", evaluations)); __sn_interpolated }
))
;
}
