...
;
    ...
    print!("{}", { let mut __sn_interpolated = String::new(); __sn_interpolated.push_str(&format!("{}", (first.clone() == copied.clone())
)); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", (first.clone() != copied.clone())
)); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", (first.clone() == second.clone())
)); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", (self::__SnClosure::<dyn Fn(i64) -> i64>(std::rc::Rc::new(same)) == first.clone())
...
;
    ...
    print!("{}", { let mut __sn_interpolated = String::new(); __sn_interpolated.push_str(&format!("{}", (identity.clone() == copiedLambda.clone())
)); __sn_interpolated.push_str(":"); __sn_interpolated.push_str(&format!("{}", (identity.clone() == first.clone())
)); __sn_interpolated.push_str("\n"); __sn_interpolated });
}
