...
fn nextFloat(trace: &mut i64, marker: i64, value: f32) -> f32 {
...
fn narrowResult(trace: &mut i64) -> f32 {
    return (((((nextFloat(&mut *(trace), 4, 1.25)) as f64) * ((2.0) as f64))) as f32);
}

fn takeFloat(value: f32) -> f32 {
    return value;
}

fn main() {
    let mut trace: i64 = 0;
    let mut stored: f32 = (((((nextFloat(&mut (trace), 1, 1.25)) as f64) * ((2.0) as f64))) as f32);
    (stored = (((((nextFloat(&mut (trace), 2, 1.25)) as f64) * ((3.0) as f64))) as f32));
    let mut argument: f32 = takeFloat((((((nextFloat(&mut (trace), 3, 1.25)) as f64) * ((4.0) as f64))) as f32));
    let mut result: f32 = narrowResult(&mut (trace));
    let mut r#box: FloatBox = FloatBox { value: 0.0 };
    ((r#box).value = (((((nextFloat(&mut (trace), 5, 1.25)) as f64) * ((5.0) as f64))) as f32));
    let mut values: Vec<f32> = vec![0.0];
    { let __sn_array_index = __sn_index((values).len(), 0); (values)[__sn_array_index] = (((((nextFloat(&mut (trace), 6, 1.25)) as f64) * ((6.0) as f64))) as f32); };
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{:.5}", stored)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x2f]))); __sn_interpolated.push_str(&format!("{:.5}", argument)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x2f]))); __sn_interpolated.push_str(&format!("{:.5}", result)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x2f]))); __sn_interpolated.push_str(&format!("{:.5}", (r#box).value)); __sn_interpolated.push_str(&(SnString::from_slice(&[0x2f]))); __sn_interpolated.push_str(&format!("{:.5}", (values)[__sn_index((values).len(), 0)])); __sn_interpolated }));
    println!("{}", trace);
}
