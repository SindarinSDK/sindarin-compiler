...
;
    let mut value: String = "base".to_string();
    let mut update: __SnClosure<dyn Fn() -> String> = { let (value, part, ) = (value.clone(), part.clone(), ); self::__SnClosure::<dyn Fn() -> String>(std::rc::Rc::new(move || -> String { let mut value = value.clone(); { let (__sn_string_part, __sn_string_place) = ((invoke(part.clone())).clone(), &mut (value)); __sn_string_place.push_str(&__sn_string_part); (*__sn_string_place).clone() };return value.clone();})) }
;
