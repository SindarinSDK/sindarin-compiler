...
fn callTwice(action: __SnClosure<dyn Fn() -> SnString>) -> SnString {
    return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(((action.clone()).0)())); __sn_interpolated.push_str(" | "); __sn_interpolated.push_str(&(((action.clone()).0)())); __sn_interpolated }
;
}

fn main() {
    std::process::exit((|| -> i64 {
        let mut greeting: SnString = SnString::from("hello world");
        let mut producer: __SnClosure<dyn Fn() -> SnString> = { let (greeting, ) = (greeting.clone(), ); self::__SnClosure::<dyn Fn() -> SnString>(std::rc::Rc::new(move || -> SnString { greeting.clone().clone()})) }
;
        __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(callTwice(producer.clone())
)); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
        __sn_print_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(((producer.clone()).0)())); __sn_interpolated.push_str("\n"); __sn_interpolated }
))
;
        return 0;
        return 0;
