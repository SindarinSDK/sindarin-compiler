...
fn callTwice(action: __SnClosure<dyn Fn() -> SnString>) -> SnString {
    return { let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&(((action.clone()).0)())); __sn_interpolated.push_str(&(SnString::from_slice(&[0x20, 0x7c, 0x20]))); __sn_interpolated.push_str(&(((action.clone()).0)())); __sn_interpolated };
}

fn main() {
    std::process::exit((|| -> i64 {
        let mut greeting: SnString = SnString::from_slice(&[0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64]);
