...
fn main() {
    ...
    if (__sn_match_subject_3.as_str() == "miss" || __sn_match_subject_3.as_str() == ((holder).direct).as_str()) {
        ...
        { let __sn_place = &mut (statementHits); let __sn_previous = *__sn_place; let __sn_next = __sn_checked(__sn_previous.checked_add(1), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_previous };
    }
};
    ...
    if (__sn_match_subject_5.as_str() == (escaped).as_str()) {
        { let __sn_place = &mut (prefix); let __sn_previous = *__sn_place; let __sn_next = __sn_checked(__sn_previous.checked_add(1), "Runtime error: integer overflow in addition"); *__sn_place = __sn_next; __sn_previous };
        ("ok".to_string())
    }
    else {
        ("bad".to_string())
    ...
    println!("{}", ((stringResult == "ok".to_string())
 && (prefix == 1)
)
);
    ...
    println!("{}", ((((holder).leaf).text == "after".to_string())
 && (empty == "".to_string())
)
);
}
