...
fn main() {
    ...
    let mut called: f64 = (bumpAndReturn(&mut (counter))
 as f64)
;
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&format!("{}", ((((((zero as f64)
 == 0.0) && ((negative as f64)
 == (-7.0))) && ((positive as f64)
 == 42.0)) && ((long_value as f64)
 == 42.0)) && ((exact_long as f64)
 == 9007199254740991.0)))); __sn_interpolated.push_str(" "); __sn_interpolated.push_str(&format!("{}", ((counter == 1) && (called == (-7.0))))); __sn_interpolated }
))
;
}
