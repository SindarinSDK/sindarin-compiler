...
fn main() {
    ...
    __sn_println_string(&({ let mut __sn_interpolated = SnString::new(); __sn_interpolated.push_str(&((source)[__sn_index((source).len(), 0)])); __sn_interpolated.push_str(&(SnString::from_slice(&[0x2c]))); __sn_interpolated.push_str(&((source)[__sn_index((source).len(), 1)])); __sn_interpolated.push_str(&(SnString::from_slice(&[0x3a]))); __sn_interpolated.push_str(&((copied)[__sn_index((copied).len(), 0)])); __sn_interpolated.push_str(&(SnString::from_slice(&[0x2c]))); __sn_interpolated.push_str(&((copied)[__sn_index((copied).len(), 1)])); __sn_interpolated }));
    { let __sn_array_index = __sn_index((source).len(), 0); (source)[__sn_array_index] = SnString::from_slice(&[0x63, 0x68, 0x61, 0x6e, 0x67, 0x65, 0x64, 0x2d, 0x73, 0x6f, 0x75, 0x72, 0x63, 0x65]); };
