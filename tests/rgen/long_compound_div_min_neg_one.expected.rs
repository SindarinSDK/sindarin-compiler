...
fn main() {
    ...
    let mut one: i64 = 1;
    let mut minimum: i64 = __sn_checked(base.checked_sub(one), "Runtime error: integer overflow in subtraction")
;
    let mut negative_one: i64 = (-1);
    { let __sn_rhs = negative_one; let __sn_place = &mut (minimum); let __sn_next = __sn_checked_div((*__sn_place).checked_div(__sn_rhs), __sn_rhs == 0); *__sn_place = __sn_next; __sn_next };
}
