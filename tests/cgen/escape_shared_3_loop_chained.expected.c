...
RtHandle __sn__build_chained(RtManagedArena *__caller_arena__) {
    ...
    RtHandle __sn__result = rt_managed_strdup(__local_arena__, RT_HANDLE_NULL, "");
    {
        ...
        while (rt_le_long(__sn__i, 2LL)) {
            {
                RtHandle __sn__outer = rt_managed_strdup(__local_arena__, RT_HANDLE_NULL, "");
                {
                    long long __sn__j = 1LL;
                    while (rt_le_long(__sn__j, 2LL)) {
                        {
                            RtHandle __sn__middle = rt_managed_strdup(__local_arena__, RT_HANDLE_NULL, "");
                            {
        ...
        char *_r = rt_str_concat(__local_arena__, "[d4:i=", _p0);
        ...
        rt_managed_strdup(__local_arena__, RT_HANDLE_NULL, _r);
                            ...
                            (__sn__outer = rt_str_concat_h(__local_arena__, RT_HANDLE_NULL, (char *)rt_managed_pin(__local_arena__, rt_str_concat_h(__local_arena__, RT_HANDLE_NULL, (char *)rt_managed_pin(__local_arena__, rt_str_concat_h(__local_arena__, RT_HANDLE_NULL, (char *)rt_managed_pin(__local_arena__, __sn__outer), "(")), (char *)rt_managed_pin(__local_arena__, __sn__middle))), ")->d2 "));
                        }
                    __for_continue_1__:;
                        rt_post_inc_long(&__sn__j);
                    }
                }
                (__sn__result = rt_str_concat_h(__local_arena__, RT_HANDLE_NULL, (char *)rt_managed_pin(__local_arena__, rt_str_concat_h(__local_arena__, RT_HANDLE_NULL, (char *)rt_managed_pin(__local_arena__, rt_str_concat_h(__local_arena__, RT_HANDLE_NULL, (char *)rt_managed_pin(__local_arena__, __sn__result), "<")), (char *)rt_managed_pin(__local_arena__, __sn__outer))), ">->d1\n"));
            }
        __for_continue_0__:;
...
int main() {
    ...
    int _return_value = 0;
    rt_println("Shared chained test (3 loops):");
