...
// Test SDK compression module
static RtUuid * __sn__moduleC__randomUuid = NULL;
...
// Test SDK compression module
static RtUuid * __sn__moduleC_1__randomUuid = NULL;
...
int main() {
    ...
    __sn__moduleC__randomUuid = __sn__UUID_create(__main_arena__);
    __sn__moduleC__randomValue = __sn__UUID_toString(__main_arena__, __sn__UUID_create(__main_arena__));
    __sn__moduleC_1__randomUuid = __sn__UUID_create(__main_arena__);
    ...
    if (__rt_interceptor_count > 0) {
        ...
        __rt_thunk_args = __args;
        __rt_thunk_arena = __local_arena__;
        RtAny __intercepted = rt_call_intercepted("test_basic_compress_decompress_A", __args, 0, __thunk_12);
    } else {
    ...
    // Each import alias creates a separate module instance, so static variables are also separate
    rt_assert(rt_ne_string((char *)rt_managed_pin(__local_arena__, __sn__UUID_toString(__local_arena__, __sn__moduleC__randomUuid)), (char *)rt_managed_pin(__local_arena__, __sn__UUID_toString(__local_arena__, __sn__moduleC_1__randomUuid))), "Should be different because each alias is a separate module instance");
