...
fn main() {
    ...
    let mut part: __SnClosure<dyn Fn() -> String> = { let (calls, ) = (calls.clone(), ); self::__SnClosure::<dyn Fn() -> String>(std::rc::Rc::new(move || -> String { { let __sn_cell = &calls; let __sn_previous = __sn_cell.get(); let __sn_next = __sn_checked_0(__sn_previous.checked_add(1), "Runtime error: integer overflow in addition"); __sn_cell.set(__sn_next); __sn_previous };return "-part".to_string();})) }
;
