...
fn main() {
    let value: std::rc::Rc<std::cell::RefCell<SnString>> = std::rc::Rc::new(std::cell::RefCell::new(SnString::from_slice(&[0x73, 0x65, 0x65, 0x64])));
...
;
    { let (__sn_value, __sn_cell) = (SnString::from_slice(&[0x6f, 0x75, 0x74, 0x65, 0x72]), &value); __sn_cell.replace(__sn_value.clone()); __sn_value };
    ...
    __sn_println_string(&(value.borrow().clone()));
    if true {
        let value: std::rc::Rc<std::cell::RefCell<SnString>> = std::rc::Rc::new(std::cell::RefCell::new(SnString::from_slice(&[0x73, 0x68, 0x61, 0x64, 0x6f, 0x77])));
        let mut shadowed: __SnClosure<dyn Fn() -> SnString> = { let (value, ) = (value.clone(), ); self::__SnClosure::<dyn Fn() -> SnString>(std::rc::Rc::new(move || -> SnString { { let (__sn_string_part, __sn_cell) = ((SnString::from_slice(&[0x2d, 0x6c, 0x6f, 0x63, 0x61, 0x6c])).clone(), &value); let mut __sn_string_place = __sn_cell.borrow_mut(); __sn_string_place.push_str(&__sn_string_part); (*__sn_string_place).clone() };return value.borrow().clone();})) }
;
