...
fn main() {
    let value: std::rc::Rc<std::cell::RefCell<SnString>> = std::rc::Rc::new(std::cell::RefCell::new(SnString::from("seed")));
...
;
    let mut first: __SnClosure<dyn Fn() -> SnString> = { let (value, ) = (value.clone(), ); self::__SnClosure::<dyn Fn() -> SnString>(std::rc::Rc::new(move || -> SnString { { let (__sn_value, __sn_cell) = ({ let mut __sn_string = SnString::new(); __sn_string.push_str(&(value.borrow().clone())); __sn_string.push_str(&(SnString::from("-set"))); __sn_string }, &value); __sn_cell.replace(__sn_value.clone()); __sn_value };{ let (__sn_string_part, __sn_cell) = ((SnString::from("-append")).clone(), &value); let mut __sn_string_place = __sn_cell.borrow_mut(); __sn_string_place.push_str(&__sn_string_part); (*__sn_string_place).clone() };return (value.borrow().clone()).to_ascii_uppercase()
 
;})) }
;
    let mut second: __SnClosure<dyn Fn() -> SnString> = { let (value, ) = (value.clone(), ); self::__SnClosure::<dyn Fn() -> SnString>(std::rc::Rc::new(move || -> SnString { { let (__sn_string_part, __sn_cell) = ((SnString::from("-sibling")).clone(), &value); let mut __sn_string_place = __sn_cell.borrow_mut(); __sn_string_place.push_str(&__sn_string_part); (*__sn_string_place).clone() };return value.borrow().clone();})) }
;
    { let (__sn_value, __sn_cell) = (SnString::from("outer"), &value); __sn_cell.replace(__sn_value.clone()); __sn_value };
...
;
    if true {
        let value: std::rc::Rc<std::cell::RefCell<SnString>> = std::rc::Rc::new(std::cell::RefCell::new(SnString::from("shadow")));
        let mut shadowed: __SnClosure<dyn Fn() -> SnString> = { let (value, ) = (value.clone(), ); self::__SnClosure::<dyn Fn() -> SnString>(std::rc::Rc::new(move || -> SnString { { let (__sn_string_part, __sn_cell) = ((SnString::from("-local")).clone(), &value); let mut __sn_string_place = __sn_cell.borrow_mut(); __sn_string_place.push_str(&__sn_string_part); (*__sn_string_place).clone() };return value.borrow().clone();})) }
;
        __sn_println_string(&(((shadowed.clone()).0)()))
;
        __sn_println_string(&(((shadowed.clone()).0)()))
;
    }
