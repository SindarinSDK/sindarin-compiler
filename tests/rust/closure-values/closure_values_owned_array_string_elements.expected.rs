...
fn makeCollector() -> __SnClosure<dyn Fn(SnString) -> SnString> {
    let mut values: Vec<SnString> = vec![SnString::from_slice(&[0x66, 0x69, 0x72, 0x73, 0x74])];
    return { let (values, ) = (std::rc::Rc::new(std::cell::RefCell::new(values.clone())), ); self::__SnClosure::<dyn Fn(SnString) -> SnString>(std::rc::Rc::new(move |value: SnString| -> SnString { { let __sn_array_value = value.clone(); values.borrow_mut().push(__sn_array_value); };return (values.borrow().clone())[__sn_index((values.borrow().clone()).len(), __sn_checked_0(((values.borrow().clone()).len() as i64).checked_sub(1), "Runtime error: integer overflow in subtraction"))].clone();})) }
;
}
