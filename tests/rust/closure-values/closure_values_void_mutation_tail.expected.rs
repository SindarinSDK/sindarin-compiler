...
;
    let mut increment: __SnClosure<dyn Fn() -> ()> = { let (count, ) = (count.clone(), ); self::__SnClosure::<dyn Fn() -> ()>(std::rc::Rc::new(move || -> () { { let __sn_cell = &count; let __sn_previous = __sn_cell.get(); let __sn_next = __sn_checked_0(__sn_previous.checked_add(1), "Runtime error: integer overflow in addition"); __sn_cell.set(__sn_next); __sn_previous };})) }
;
    ((add.clone()).0)();
    ((increment.clone()).0)();
    println!("{}", count.get());
