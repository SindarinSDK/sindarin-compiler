...
static void *__thread_wrapper_0__(void *arg) {
    ...
    long long __result__;
    if (__rt_interceptor_count > 0) {
        RtAny __args[1];
        __rt_thunk_args = __args;
        __rt_thunk_arena = __arena__;
        RtAny __intercepted = rt_call_intercepted(rt_arena_v2_strdup(__arena__, "compute"), __args, 0, __thread_thunk_0);
        __result__ = rt_unbox_int(__intercepted);
    } else {
        __result__ = __sn__compute(__arena__);
    }
...
int main() {
    ...
    long long __sn__result = ({ RtHandleV2 *__sync_h__ = rt_thread_v3_sync(({
    ...
    __spawn_handle_0__;
})); rt_handle_begin_transaction(__sync_h__); long long __sync_val__ = *(long long *)__sync_h__->ptr; rt_handle_end_transaction(__sync_h__); __sync_val__; });
    RtHandleV2 *__htmp_0__ = rt_arena_v2_strdup(__local_arena__, "expected result to be 42");
    RtHandleV2 *__htmp_1__ = rt_arena_v2_strdup(__local_arena__, "expected result to be 42");
    rt_assert_v2(rt_eq_long(__sn__result, 42LL), __htmp_1__);
    goto main_return;
main_return:
...
static RtAny __thread_thunk_0(void) {
    RtAny __result = rt_box_int(__sn__compute((RtArenaV2 *)__rt_thunk_arena));
    return __result;
}
