...
int main() {
    ...
    if (__handle_pending__ != NULL) {
        RtHandleV2 *__sync_h__ = rt_thread_v3_sync(__handle_pending__);
        rt_handle_begin_transaction(__sync_h__);
        __sn__handle = *(long long *)__sync_h__->ptr;
        rt_handle_end_transaction(__sync_h__);
        __handle_pending__ = NULL;
    }
    __sn__handle;
});
